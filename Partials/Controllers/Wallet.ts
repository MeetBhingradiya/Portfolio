...
export class WalletController {
    ...
    static async deleteWalletTransaction(transactionId: string, userID: string) {
        try {
            ...
            // Reverse the wallet balance change based on transaction type
            if (transaction.FinancialType === 'TRANSFER') {
                // For transfers, reverse both wallets
                if (transaction.FromWalletID) {
                    // Find the source wallet (could be pocket or UPI)
                    const fromWallet = await Wallets_Model.findOne({ WalletID: transaction.FromWalletID });
                    
                    if (fromWallet) {
                        // If it's a UPI wallet, update the linked pocket instead
                        if (fromWallet.Type === 'UPI' && fromWallet.LinkedWalletID) {
                            await Wallets_Model.updateOne(
                                { WalletID: fromWallet.LinkedWalletID },
                                { $inc: { Balance: transaction.Amount } } // Add back the amount sent
                            );
                        } else {
                            // Regular wallet or pocket
                            await Wallets_Model.updateOne(
                ...
                if (transaction.ToWalletID) {
                    // Find the destination wallet (could be pocket or UPI)
                    const toWallet = await Wallets_Model.findOne({ WalletID: transaction.ToWalletID });
                    
                    if (toWallet) {
                        // If it's a UPI wallet, update the linked pocket instead
                        if (toWallet.Type === 'UPI' && toWallet.LinkedWalletID) {
                            await Wallets_Model.updateOne(
                                { WalletID: toWallet.LinkedWalletID },
                                { $inc: { Balance: -transaction.Amount } } // Subtract the amount received
                            );
                        } else {
                            // Regular wallet or pocket
                            await Wallets_Model.updateOne(
            ...
            } else if (transaction.FinancialType === 'INCOME') {
                // For income, subtract the amount (reverse credit)
                const toWallet = await Wallets_Model.findOne({ WalletID: transaction.ToWalletID });
            ...
            } else if (transaction.FinancialType === 'EXPENSE') {
                // For expense, add back the amount (reverse debit)
                const fromWallet = await Wallets_Model.findOne({ WalletID: transaction.FromWalletID });
