...
export async function POST(request: NextRequest) {
    // Disabled: The new user schema uses a single email field instead of Emails array
    // This endpoint needs to be refactored to support email changes instead of adding secondary emails
    return NextResponse.json(
        {
            Status: 0,
            Message: "Adding secondary emails is not currently supported. Please contact support to change your primary email.",
            StatusCode: 501
        },
        { status: 501 }
    );
    /* Original code disabled - incompatible with new schema
    try {
        ...
        const updatedUser = await EnhancedUsers_Model.findOneAndUpdate(
            ...
            { new: true }
        ).select("email profile.firstName password");
        ...
        // Note: Email verification needs to be refactored for new schema
        // The new schema uses a single email field instead of Emails array
        log(
...
export async function PATCH(request: NextRequest) {
    return NextResponse.json(
        {
            Status: 0,
            Message: "Email management is not currently supported. Please contact support to change your email.",
            StatusCode: 501
        },
        { status: 501 }
    );
    /* Disabled - incompatible with new schema
    try {
        const authHeader = request.headers.get("authorization");
        const token =
            authHeader?.replace("Bearer ", "") ||
        ...
        if (setPrimary) {
            ...
            const result = await EnhancedUsers_Model.updateOne(
                {
                    "UserID": decoded.userID,
                    "Emails.Email": email
                },
...
export async function DELETE(request: NextRequest) {
    return NextResponse.json(
        {
            Status: 0,
            Message: "Email management is not currently supported. Please contact support.",
            StatusCode: 501
        },
        { status: 501 }
    );
