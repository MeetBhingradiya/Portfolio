...
function CreateUsernameContent() {
    ...
    const handleCreateUsername = async () => {
        ...
        try {
            ...
            if (response.data.Status === 1) {
                setSuccess(
                    "Username created successfully! Redirecting to dashboard..."
                );
            ...
            setError(
                err.response?.data?.Message || "Failed to create username"
            );
        } finally {
            setLoading(false);
        }
    ...
    const getUsernameValidationStatus = () => {
        ...
        if (username.length < 3)
            return { color: "error", message: "Too short (min 3 characters)" };
        if (checking)
            return { color: "info", message: "Checking availability..." };
        if (usernameAvailable === true)
            return { color: "success", message: "Available!" };
        if (usernameAvailable === false)
            return { color: "error", message: "Not available" };
        return null;
    };
                ...
                <Card variant="outlined">
                    <CardContent>
                        ...
                        <Typography
                            ...
                            It must be unique and at least 3 characters long.
                        </Typography>
