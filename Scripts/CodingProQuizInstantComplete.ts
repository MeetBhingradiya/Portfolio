...
const Cookie =
    "session_id=39555802-81f8-4467-87c8-bfdb1d6ad022; token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJjYXRlZ29yeV9pZCI6Ils0NzddIiwiZW1haWwiOiIyMklEMDFJVDAwNSIsImV4cCI6MTc0NjA3MDE3MiwiaXNfdW5pdiI6MSwicm9sZV9pZCI6Mywic2Nob29sX2lkIjozMDcyLCJ1c2VyX2lkIjozMjQ0MCwidXNlcl9uYW1lIjoiQmhpbmdyYWRpeWEgTWVldCBQYXJlc2hiaGFpIn0.k7hR9mCURJFGCtcjIq96hRhHqL9IZwbU-uEp7E3s5Do";
const Authorization =
...
const APIs = {
    ...
    getCourse:
        "https://codingpro.online/api/v1/student_dash/courses/course?class=@class&subject=@subject",
...
async function getCourse(): Promise<CourseResponse> {
    const url = APIs.getCourse
        .replace("@class", CourseDetails.class.toString())
        .replace("@subject", CourseDetails.subject.toString());
...
async function getQuizData(quizId: number): Promise<QuizQuestionsResponse> {
    ...
    const response = await Axios.get<QuizQuestionsResponse>(url, {
        headers: Headers
    });
    return response.data;
}
...
async function getQuizResult(quizId: number): Promise<QuizCompletionStatus> {
    ...
    const response = await Axios.get<QuizCompletionStatus>(url, {
        headers: Headers
    });
    return response.data;
}
    ...
    try {
        ...
        for (const quiz of quizzes) {
            ...
            try {
                ...
                if (
                    quizResult.is_completed &&
                    quizResult.correct_answers !== undefined &&
                    quizResult.total_questions !== undefined &&
                    quizResult.correct_answers !==
                        quizResult.total_questions - 1
                ) {
                    console.log(
                        `Quiz was completed with ${quizResult.correct_answers}/${quizResult.total_questions} correct answers.`
                    );
                    console.log(
                        `This is not a near-perfect score. Submitting result again...`
                    );
                    ...
                    console.log(
                        `Quiz score improved successfully: ${saveResult.message}`
                    );

                    // Add delay to avoid rate limiting
                    console.log(
                        "Waiting 3 seconds before processing next quiz..."
                    );
                    await sleep(3000);
                    continue;
                }
                ...
                if (quizResult.is_completed) {
                    console.log(
                        `Quiz already completed with ${quizResult.correct_answers}/${quizResult.total_questions} correct answers.`
                    );
                ...
                console.log(
                    "Quiz not completed yet. Fetching quiz questions..."
                );
                ...
                console.log(
                    `Quiz has ${totalQuestions} questions. Submitting result...`
                );
                ...
                console.log(
                    `Quiz completed successfully: ${saveResult.message}`
                );
