...
if (!ENCRYPTION_KEY_HEX || !SIGNING_SECRET) {
    throw new Error(
        "Missing ENCRYPTION_KEY or SERVICE_SIGNING_SECRET env variables"
    );
}
...
app.use((req, res, next) => {
    ...
    if (ALLOWED_ORIGINS.length && origin && ALLOWED_ORIGINS.includes(origin)) {
        ...
        res.setHeader(
            ...
            "Content-Type, x-signature, x-timestamp"
        );
...
const requireSignature = (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (!isValid) {
            console.error(
                `[Security] Invalid signature from IP: ${req.ip} at ${new Date().toISOString()}`
            );
            return res
                .status(403)
...
app.post("/encrypt", requireSignature, (req: Request, res: Response) => {
    ...
    try {
        ...
        console.log(
            `[Metrics] Encryption completed in ${duration}ms for IP: ${req.ip}`
        );

        return res.json({ success: true, result: encrypted });
    } catch (err: any) {
        console.error(
            `[Error] Encryption failed for IP ${req.ip}:`,
            err.message
        );
...
app.post("/decrypt", requireSignature, (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!data.includes(":") || data.split(":").length !== 3) {
            return res
                .status(400)
                .json({ error: "Invalid encrypted data format" });
        }
        ...
        const duration = Date.now() - startTime;
        console.log(
            `[Metrics] Decryption completed in ${duration}ms for IP: ${req.ip}`
        );
        ...
        console.error(
            `[Error] Decryption failed for IP ${req.ip}:`,
            err.message
        );
        // Generic error to avoid leaking implementation details
        return res
            .status(500)
            .json({ error: "Decryption failed or invalid data" });
    }
});
...
app.use((req, res) => {
    console.warn(
        `[Security] 404 - Undefined route accessed: ${req.method} ${req.path} from ${req.ip}`
    );
...
if (process.env.NODE_ENV !== "production" || process.env.VERCEL !== "1") {
    app.listen(PORT, () => {
        console.log(
            `[Server] Encryption Service running on http://localhost:${PORT}`
        );
        console.log(
            `[Server] Environment: ${process.env.NODE_ENV || "development"}`
        );
        console.log(
            `[Server] CORS Origins: ${ALLOWED_ORIGINS.join(", ") || "None (disabled)"}`
        );
    });
}
