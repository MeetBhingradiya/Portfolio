...
export const OneUICard: React.FC<OneUICardProps> = ({ children, className = "", elevated = false, interactive = true }) => {
...
export const OneUIHeader: React.FC<OneUIHeaderProps> = ({ title, subtitle, className = "" }) => {
    const { palette, actualColorMode } = useDesignTheme();
    const isDark = actualColorMode === "dark";
...
export const OneUIBadge: React.FC<OneUIBadgeProps> = ({ children, variant = "accent", className = "" }) => {
    const { palette, actualColorMode } = useDesignTheme();
    const isDark = actualColorMode === "dark";
...
export const OneUIListItem: React.FC<OneUIListItemProps> = ({ title, subtitle, icon, rightContent, onClick, className = "" }) => {
...
export const OneUITabs: React.FC<OneUITabsProps> = ({ tabs, activeTab, onChange, className = "" }) => {
    const { palette } = useDesignTheme();
        ...
        <div
            ...
            {tabs.map((tab) => (
                <motion.button
                    ...
                    style={{
                        background: activeTab === tab.value ? palette.accent : "transparent",
                        color: activeTab === tab.value ? palette.textOnAccent : palette.textSecondary
                    }}
...
export const OneUISection: React.FC<OneUISection> = ({ children, className = "", spacing = "comfortable" }) => {
    const { palette } = useDesignTheme();
...
export const OneUIDivider: React.FC<OneUIDividerProps> = ({ text, className = "" }) => {
    const { palette } = useDesignTheme();

    return (
        <div className={`oneui-divider flex items-center gap-4 my-8 ${className}`}>
            <div
