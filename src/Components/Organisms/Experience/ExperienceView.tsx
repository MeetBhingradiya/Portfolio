...
import {
    Work, School, VerifiedUser, Code, OpenInNew,
    LocationOn, Star, KeyboardArrowDown, KeyboardArrowUp, Download
...
type SkillCategory =
    ...
    | "cloud" | "tools" | "design" | "soft_skills" | "other";
...
function SectionHeading({ icon, title, palette, isApple }: {
    icon: React.ReactNode; title: string; palette: any; isApple: boolean;
}) {
    return (
        <div className="flex items-center gap-3 mb-6">
            <span style={{ color: palette.accent }}>{icon}</span>
            <h2 className={isApple ? "text-2xl font-bold" : "text-3xl font-black"}
...
function ExperienceSection({ experience, palette, isDark, isApple }: {
    experience: Experience[]; palette: any; isDark: boolean; isApple: boolean;
}) {
    ...
    return (
        <section className="mb-16">
            <SectionHeading icon={<Work />} title="Work Experience" palette={palette} isApple={isApple} />
            <div className="flex flex-col gap-5">
                {experience.map((exp, i) => {
                    ...
                    return (
                        <motion.div key={exp.ExperienceID} style={cardStyle}
                            ...
                            <div className="absolute inset-y-0 left-0 w-1"
                                style={{ background: exp.CurrentlyWorking ? "#30D158" : "#0A84FF" }} />
                            ...
                            <div className="pl-5 pr-5 py-5 relative z-10">
                                ...
                                <div className="flex items-start justify-between flex-wrap gap-3">
                                    ...
                                    <div className="text-right shrink-0">
                                        <p className="text-xs font-semibold" style={{ color: palette.textTertiary }}>
                                ...
                                <div className="flex flex-wrap items-center gap-2 mt-3">
                                    ...
                                    {exp.CurrentlyWorking && (
                                        <span className="text-xs px-2 py-0.5 rounded-full font-bold animate-pulse"
                                            style={{ background: "#30D15820", color: "#30D158" }}>● Current</span>
                                    )}
                                    {exp.Location && (
                                        <span className="flex items-center gap-1 text-xs" style={{ color: palette.textTertiary }}>
                                            <LocationOn style={{ fontSize: 12 }} />{exp.Location} · {exp.LocationType}
                                        </span>
                                    )}
                                </div>
                                    ...
                                    <div className="flex flex-wrap gap-1.5 mt-3">
                                        {exp.TechStack.map(t => (
                                            <span key={t} className="px-2 py-0.5 rounded-full text-xs"
                                                style={{ background: "#0A84FF14", color: "#0A84FF", border: "0.5px solid #0A84FF28" }}>
                                                {t}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                {/* Expand toggle */}
                                {hasDetails && (
                                    <button onClick={() => toggle(exp.ExperienceID)}
                                        className="flex items-center gap-1 text-xs font-semibold mt-4 hover:opacity-80"
                                        style={{ color: palette.accent }}>
                                        {isExp ? <><KeyboardArrowUp fontSize="small" /> Less details</> : <><KeyboardArrowDown fontSize="small" /> More details</>}
                                    </button>
                                )}
                                        ...
                                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
                                            ...
                                            <div className="mt-4 grid md:grid-cols-2 gap-4">
                                                {exp.Responsibilities.length > 0 && (
                                                    <div>
                                                        <p className="text-xs font-black uppercase tracking-widest mb-2" style={{ color: palette.textTertiary }}>Responsibilities</p>
                                                        <ul className="space-y-1.5">
                                                            {exp.Responsibilities.map((r, i) => (
                                                                <li key={i} className="text-sm flex gap-2" style={{ color: palette.textSecondary }}>
                                                                    <span style={{ color: "#0A84FF", shrink: 0 } as any}>▸</span>{r}
                                                                </li>
                                                            ))}
                                                        </ul>
                                                    </div>
                                                ...
                                                {exp.Achievements.length > 0 && (
                                                    <div>
                                                        <p className="text-xs font-black uppercase tracking-widest mb-2" style={{ color: palette.textTertiary }}>Achievements</p>
                                                        <ul className="space-y-1.5">
                                                            {exp.Achievements.map((a, i) => (
                                                                <li key={i} className="text-sm flex gap-2" style={{ color: palette.textSecondary }}>
                                                                    <Star style={{ fontSize: 14, color: "#FF9F0A", flexShrink: 0 }} />{a}
                                                                </li>
                                                            ))}
                                                        </ul>
                                                    </div>
...
function SkillsSection({ skills, palette, isDark, isApple }: {
    skills: Skill[]; palette: any; isDark: boolean; isApple: boolean;
}) {
                                    ...
                                    <div key={skill.SkillID}>
                                        ...
                                        <div className="h-1.5 rounded-full overflow-hidden"
                                            ...
                                            <motion.div className="h-full rounded-full"
                                                style={{ background: skill.Color ?? color }}
                                                initial={{ width: 0 }}
...
function EducationSection({ education, palette, isDark, isApple }: {
    education: Education[]; palette: any; isDark: boolean; isApple: boolean;
}) {
    const cardStyle: React.CSSProperties = {
        background: isApple
            ? isDark ? "linear-gradient(180deg,rgba(58,58,60,0.6) 0%,rgba(44,44,46,0.55) 100%)"
    ...
    return (
        <section className="mb-16">
            <SectionHeading icon={<School />} title="Education" palette={palette} isApple={isApple} />
            <div className="flex flex-col gap-5">
                {education.map((edu, i) => (
                    <motion.div key={edu.EducationID} style={cardStyle}
                        initial={{ opacity: 0, y: 16 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        viewport={{ once: true }}
                        transition={{ duration: 0.45, delay: i * 0.07 }}>
                        <div className="absolute inset-y-0 left-0 w-1" style={{ background: "#30D158" }} />
                        <div className="pl-5 pr-5 py-5">
                            <div className="flex items-start justify-between flex-wrap gap-3">
                                <div className="flex items-start gap-3">
                                    {edu.Logo ? (
                                        // eslint-disable-next-line @next/next/no-img-element
                                        <img src={edu.Logo} alt={edu.Institution} className="w-10 h-10 rounded-xl object-cover border"
                                            style={{ borderColor: isDark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.08)" }} />
                                    ) : (
                                        <div className="w-10 h-10 rounded-xl flex items-center justify-center text-base font-black"
                            ...
                            {edu.Location && (
                                <div className="flex items-center gap-1 mt-2" style={{ color: palette.textTertiary }}>
                            ...
                            {edu.Achievements.length > 0 && (
                                <ul className="mt-3 space-y-1">
                                    {edu.Achievements.map((a, j) => (
                                        <li key={j} className="text-sm flex gap-2" style={{ color: palette.textSecondary }}>
                                            <Star style={{ fontSize: 14, color: "#FF9F0A", flexShrink: 0 }} />{a}
                                        </li>
                                    ))}
                                </ul>
...
function CertificatesSection({ certificates, palette, isDark, isApple }: {
    certificates: Certificate[]; palette: any; isDark: boolean; isApple: boolean;
}) {
    const cardStyle: React.CSSProperties = {
        background: isApple
            ? isDark ? "linear-gradient(180deg,rgba(58,58,60,0.6) 0%,rgba(44,44,46,0.55) 100%)"
    ...
    return (
        <section className="mb-16">
            <SectionHeading icon={<VerifiedUser />} title="Licences & Certifications" palette={palette} isApple={isApple} />
            <div className="grid md:grid-cols-2 gap-5">
                {certificates.map((cert, i) => (
                    <motion.div key={cert.CertificateID} style={cardStyle}
                        initial={{ opacity: 0, y: 16 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        viewport={{ once: true }}
                        transition={{ duration: 0.4, delay: i * 0.06 }}>
                        <div className="absolute inset-y-0 left-0 w-1" style={{ background: "#BF5AF2" }} />
                        <div className="pl-5 pr-5 py-5">
                            <div className="flex items-start gap-3">
                                {cert.Logo ? (
                                    // eslint-disable-next-line @next/next/no-img-element
                                    <img src={cert.Logo} alt={cert.IssuingOrganization}
                                        className="w-10 h-10 rounded-xl object-cover border"
                                        style={{ borderColor: isDark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.08)" }} />
                                ) : (
                                ...
                                <div className="flex-1 min-w-0">
                                    <h3 className={isApple ? "text-sm font-semibold leading-snug" : "text-base font-bold leading-snug"}
                                        style={{ color: palette.textPrimary }}>
                                        {cert.Title}
                                    </h3>
                                    <p className="text-xs font-medium mt-0.5" style={{ color: "#BF5AF2" }}>
                            ...
                            {cert.Description && (
                                <p className="mt-3 text-xs leading-relaxed" style={{ color: palette.textSecondary }}>
                                    {cert.Description}
                                </p>
                            )}
...
export default function ExperienceView({ experience, skills, education, certificates }: Props) {
    ...
    const totalYears = useMemo(() => {
        const months = experience.reduce((sum, e) =>
            sum + monthsDiff(e.StartDate, e.EndDate, e.CurrentlyWorking), 0);
    ...
    const stats = [
        { label: "Years of Experience", value: `${totalYears}+`, color: "#0A84FF" },
        { label: "Companies",           value: experience.length,  color: "#30D158" },
        { label: "Skills",              value: skills.length,      color: "#BF5AF2" },
        { label: "Certifications",      value: certificates.length, color: "#FF9F0A" }
    ];
    ...
    return (
        <div className="min-h-screen pt-24 pb-20">
            <div className="max-w-5xl mx-auto px-4 md:px-8">
                ...
                {/* Stats overview */}
                <motion.div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-14"
                    initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}>
                    {stats.map((s, i) => (
                        <div key={s.label} style={statCardBg} className="p-5 text-center">
                            <p className={isApple ? "text-3xl font-bold" : "text-4xl font-black"} style={{ color: s.color }}>
                                {s.value}
                            </p>
                            <p className="text-xs mt-1" style={{ color: palette.textTertiary }}>{s.label}</p>
                        </div>
                    ))}
                </motion.div>

                {experience.length > 0 && (
                    <ExperienceSection experience={experience} palette={palette} isDark={isDark} isApple={isApple} />
                )}
                {skills.length > 0 && (
                    <SkillsSection skills={skills} palette={palette} isDark={isDark} isApple={isApple} />
                )}
                {education.length > 0 && (
                    <EducationSection education={education} palette={palette} isDark={isDark} isApple={isApple} />
                )}
                {certificates.length > 0 && (
                    <CertificatesSection certificates={certificates} palette={palette} isDark={isDark} isApple={isApple} />
                )}
            </div>
        </div>
    );
