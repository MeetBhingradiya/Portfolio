...
export class MemoryStore implements Store {
    ...
    async get(key: string): Promise<ClientRateLimitInfo | undefined> {
        ...
        return client
            ? { totalHits: client.totalHits, resetTime: client.resetTime }
            : undefined;
    }
    ...
    private getClientFromStorage(key: string): Client | undefined {
        ...
        if (this.currentIndex.has(key)) {
            return this.deserializeClient(
                this.current,
                this.currentIndex.get(key)!
            );
        }

        // Check previous storage
        if (this.previousIndex.has(key)) {
            const client = this.deserializeClient(
                this.previous,
                this.previousIndex.get(key)!
            );
    ...
    private deleteClient(key: string, storage: "current" | "previous"): void {
        const index =
            storage === "current" ? this.currentIndex : this.previousIndex;
    ...
    private deserializeClient(buffer: Buffer, position: number): Client {
        ...
        const resetTimeMs = Number(
            buffer.readBigUInt64BE(clientDataPosition + 4)
        );
    ...
    private rebuildIndex(storage: "current" | "previous"): void {
        ...
        while (position < buffer.length) {
            const keyLength = buffer.readUInt32BE(position);
            const key = buffer
                .subarray(position + 4, position + 4 + keyLength)
                .toString("utf8");
