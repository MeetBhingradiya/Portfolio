...
const RateLimitPolicySchema = new Schema<IRateLimitPolicy>(
    {
        ...
        maxFileSizeBytes:  { type: Number, required: true, default: 10 * 1024 * 1024 }, // 10 MB default
...
const CDNAPIKeySchema = new Schema<ICDNAPIKey>(
    {
        keyId:          { type: String, required: true, unique: true, index: true, trim: true },
        keyHash:        { type: String, required: true, unique: true, index: true, trim: true },
...
export const CDNAPIKey =
    mongoose.models.CDNAPIKey ||
    mongoose.model<ICDNAPIKey>("CDNAPIKey", CDNAPIKeySchema);
