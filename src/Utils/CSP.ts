...
function CSPGenerator(options: CSPGeneratorOptions = {}): string {
    ...
    const serializeDirective = (
        ...
        if (value.Domains && value.Domains.length > 0) {
            ...
            const invalidDomains = value.Domains.filter(
                (domain: string) =>
                    // ? Add * as Valid Domain
    ...
    const directives = Object.entries(directive).map(([key, value]) =>
        serializeDirective(
            key as CSPDirectiveOptions,
            value as DirectiveOptions
        )
    );
    ...
    const csp = [...directives, ...defaultDirectives]
        .filter(Boolean)
        .join("; ");
...
export { CSPGenerator, CSPDirectiveOptions };
export type { CSPGeneratorOptions, DirectiveOptions };
