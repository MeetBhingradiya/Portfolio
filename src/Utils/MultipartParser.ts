...
export async function parseMultipartResponse(response: Response): Promise<{
    ...
    while (boundaryIndex !== -1) {
        ...
        if (
            position + 1 < uint8Array.length &&
            uint8Array[position] === 0x0d &&
            uint8Array[position + 1] === 0x0a
        ) {
            position += 2;
        }

        // Find the next boundary
        const nextBoundaryIndex = findBytes(
            uint8Array,
            boundaryBytes,
            position
        );
    ...
    for (const part of parts) {
        if (part.name === "metadata") {
            ...
            if (indexMatch) {
                ...
                // Create a copy of the data as a plain ArrayBuffer to avoid type issues
                const arrayBuffer = new ArrayBuffer(part.data.length);
