...
class SessionManager {
    ...
    cleanupInvalidSession(userID?: string): void {
        ...
        try {
            ...
            const targetUserID = userID || this.getCurrentUserID(currentToken, accounts);
            
            if (targetUserID) {
                // Remove the invalid account
                const updatedAccounts = accounts.filter((acc: any) => acc.userID !== targetUserID);
                
                // If this was the current account
                if (currentToken && accounts.find((acc: any) => acc.userID === targetUserID && acc.encryptedToken === currentToken)) {
                    // Clear current token
                    ...
                    if (updatedAccounts.length > 0) {
                        const mostRecentAccount = updatedAccounts.sort((a: any, b: any) => 
                            new Date(b.lastUsed || 0).getTime() - new Date(a.lastUsed || 0).getTime()
                        )[0];
                        
                        localStorage.setItem('auth-token', mostRecentAccount.encryptedToken);
    ...
    private getCurrentUserID(token: string | null, accounts: any[]): string | null {
        if (!token || !accounts.length) return null;
        
        const currentAccount = accounts.find(acc => acc.encryptedToken === token);
    ...
    handle401Error(): void {
        ...
        if (currentToken) {
            ...
            if (savedAccounts) {
                try {
                    const accounts = JSON.parse(savedAccounts);
                    const invalidAccount = accounts.find((acc: any) => acc.encryptedToken === currentToken);
                    if (invalidAccount) {
                        this.cleanupInvalidSession(invalidAccount.userID);
                        return;
                    }
                } catch (error) {
                    console.error('Error parsing saved accounts during 401 handling:', error);
                }
            }
        }
...
export const handle401Error = () => 
    sessionManager.handle401Error();

export const clearAllAccountData = () => 
    sessionManager.clearAllAccountData();
