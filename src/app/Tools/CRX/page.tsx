...
const CRX_URL = "https://clients2.google.com/service/update2/crx?response=redirect&acceptformat=crx3&prodversion=@PRODVERSION&x=id%3D@EXTENSIONID%26installsource%3Dondemand%26uc&nacl_arch=@NACL_ARCH";
// Extension ZIP download URL format (same as CRX but handled differently on the client)
const ZIP_URL = "https://clients2.google.com/service/update2/crx?response=redirect&acceptformat=crx3&prodversion=@PRODVERSION&x=id%3D@EXTENSIONID%26installsource%3Dondemand%26uc&nacl_arch=@NACL_ARCH";
...
const DEFAULT_ICON = "https://www.google.com/chrome/static/images/chrome-logo.svg";
...
function CRXDownload() {
    ...
    function fetchVersions() {
        ...
        if (!state.extension && !state.error) {
            alert("Please search for an extension first before attempting to download.");
            return;
        }
        ...
        const fileName = state.extension?.name
            ? state.extension.name.replace(/[&\/\\#,+()$~%.'":*?<>|{}\s]/g, '-').replace(/-*$/g, '').replace(/-+/g, '-')
            : state.id;
    ...
    async function convertURLToZip(url: string) {
        ...
        try {
            ...
            alert("Failed to convert to ZIP format. Trying direct download instead.");
    ...
    async function fetchExtensionInfo() {
        ...
        try {
            ...
            if (!proxyResponse || proxyData.error || proxyResponse.status >= 400) {
                setState({
                    ...state,
                    isLoading: false,
                    error: "Extension not found in Chrome Web Store"
                });
                console.error("Error in CORS proxy response:", proxyData.error || "HTTP status " + proxyResponse.status);
                return;
            }
            ...
            const nameElement = doc.querySelector('h1') || doc.querySelector('meta[property="og:title"]');
            ...
            if (iconElement) {
                ...
                if (iconElement.tagName === 'IMG') {
                    ...
                    if (icon && !icon.startsWith('http')) {
                        icon = new URL(icon, WEBSTORE_URL.replace("@EXTENSIONID", state.id)).href;
                    }
                    ...
                    icon = iconElement.getAttribute('content') ||
            ...
            if (icon === DEFAULT_ICON || !icon.startsWith('http')) {
                ...
                for (const iconUrl of iconPatterns) {
                    try {
                        ...
                        const proxyIconResponse = await Axios.post('/api/cors', {
                            body: {
                                endpoint: iconUrl,
                                method: 'HEAD'
                            }
                        ...
                        if (proxyIconResponse.status === 200) {
                            icon = iconUrl;
                            console.log("Found working icon URL via CORS proxy:", icon);
                            break;
                        }
                    } catch (err) {
                        console.error(`Error checking icon URL ${iconUrl}:`, err);
                    }
                }
            ...
            console.log("Found structured data scripts:", scriptElements.length);
            ...
            const jsonLinkedData = doc.querySelector('script[type="application/ld+json"]');
            const userCountElement = doc.querySelector('[title*="user"], [aria-label*="user"], [data-users]');
            const ratingElement = doc.querySelector('[aria-label*="rating"], [data-rating]');
            ...
            const rating = ratingElement ?
                ...
                "N/A"; setState({
                    ...
                    extension: {
                        ...
                        description: structData?.description || description || "No description available",
                        icon: structData?.image || icon || DEFAULT_ICON,
                        users: structData?.interactionCount || usersCount || "Unknown",
                        rating: structData?.aggregateRating?.ratingValue || rating || "N/A",
            ...
            console.log("Successfully fetched extension info:", {
                id: state.id,
                icon: structData?.image || icon || DEFAULT_ICON,
                iconSource: structData?.image ? "structured data" : (icon !== DEFAULT_ICON ? "HTML extraction" : "default icon")
            });
        } catch (err) {
                    ...
                    <div className="utility-content">
                        <div className="extension-card">
                            <div className="extension-icon">
                                <img
                                    ...
                                    onError={(e) => {
                                        ...
                                        console.log("Icon failed to load, using default:", e);
                            ...
                            <div className="extension-details">
                                ...
                                <p className="extension-description">{state.extension.description}</p>
                                ...
                                {state.extension.website && (
                                    <div className="extension-website">
                                        <Button
                                            startContent={<Public />}
                                            onPress={() => state.extension?.website && window.open(state.extension.website)}
                                        >
                                            Visit Store
                                        </Button>
                                    </div>
            ...
            <div className="utility-box glass">
                <div className="utility-header">
                    ...
                    <div className="installation-steps">
                        ...
                        <div className="install-method">
                            <h5>Method 2: Install Unpacked Extension (ZIP File)</h5>
                            <ol>
                        ...
                        <div className="tips-section">
                            ...
                            <ul>
                                ...
                                <li>Keep extensions from trusted sources only</li>
                            </ul>
                        </div>
                    </div>
                </div>
