...
export default function ImageToPDF() {
    ...
    const handleFiles = async (files: FileList | File[]) => {
        ...
        if (images.length + fileArray.length > MAX_FILES) {
            alert(
                `Maximum ${MAX_FILES} files allowed. You can add ${MAX_FILES - images.length} more files.`
            );
            return;
        }
    ...
    const handleDragStart = (e: React.DragEvent, index: number) => {
        ...
        const actualIndex = images.findIndex(
            (img) => img.id === sortedImages[index].id
        );
    ...
    const handleDragOverItem = (e: React.DragEvent, index: number) => {
        ...
        if (draggedIndex !== null) {
            const sortedImages = [...images].sort((a, b) => a.order - b.order);
            const actualDropIndex = images.findIndex(
                (img) => img.id === sortedImages[index].id
            );
    ...
    return (
        <div
            className="flex flex-col items-center w-full max-w-5xl mx-auto px-4 py-6 pt-45
        ...
        min-h-max pb-16">
            ...
            <div className="text-base text-gray-400 mb-8 text-center">
                Convert your images to PDF with customizable settings. Supports
                ...
                <div
                    ...
                    <div className="text-center text-white">
                        ...
                        <div className="text-sm text-gray-400 mb-1">
                            Supports: JPEG, PNG, WebP, TIFF, AVIF, BMP, ICO,
                            HEIC, HEIF, SVG
                        </div>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4">
                        ...
                        <div className="flex items-center gap-4">
                            <div className="flex border border-white/20 rounded overflow-hidden">
                                <button
                                    className={`p-2 bg-white/10 border-none text-gray-400 cursor-pointer transition-all duration-300 flex items-center justify-center hover:bg-white/20 hover:text-white ${
                                        viewMode === "list"
                        ...
                        <div className="flex gap-2 flex-wrap">
                            <button
                                className={`px-4 py-2 rounded border border-white/20 bg-white/10 text-white cursor-pointer transition-all duration-300 flex items-center gap-2 text-sm font-medium hover:bg-white/20 hover:border-white/30 disabled:opacity-50 disabled:cursor-not-allowed ${
                                    isProcessing
                            ...
                            .map((image, sortedIndex) => {
                                ...
                                return (
                                    <div
                                        ...
                                        } bg-white/5 border border-white/10 rounded-lg transition-all duration-300 cursor-move hover:bg-white/10 hover:border-white/20 hover:-translate-y-0.5 hover:cursor-grab active:cursor-grabbing ${
                                        ...
                                        draggable
                                        onDragStart={(e) =>
                                            handleDragStart(e, sortedIndex)
                                        }
                                        onDragEnd={handleDragEnd}
                                        onDragOver={(e) =>
                                            handleDragOverItem(e, sortedIndex)
                                        }
                                        onDrop={(e) =>
                                            handleDropItem(e, sortedIndex)
                                        }>
                                        {" "}
                                        <div
                                            className={`relative ${
                                        ...
                                        <div
                                            className={`flex-1 min-w-0 ${viewMode === "grid" ? "text-center p-3" : ""}`}>
                                            <div
                                                className="text-white font-medium mb-1 overflow-hidden text-ellipsis whitespace-nowrap"
                                                title={image.file.name}>
                                                {image.file.name}
                                            </div>
                                            <div
                                                className={`flex items-center gap-4 text-xs ${viewMode === "grid" ? "justify-center" : ""}`}>
                                                ...
                                                <span
                                                    className={`px-1.5 py-0.5 rounded font-medium text-xs capitalize ${
                                                        image.status ===
                                                        "pending"
