...
function Tools() {
	...
	const [ModalState, SetModalState] = React.useState<IToolsModalData>(
		DefualtToolsModalData
	);
	...
	const [SuggestionsState, setSuggestionsState] =
		React.useState<IToolsSuggestionsState>(DefualtToolsSuggestionsState);
	...
	const handleKeyPress = (
		...
		if (e.key === "ArrowDown") {
			e.preventDefault();
			if (
				SuggestionsState.Index + 1 <=
				SuggestionsState.Suggestions.length
			) {
				setSuggestionsState(
					(SuggestionsState: IToolsSuggestionsState | any) => {
						return {
							...SuggestionsState,
			...
			if (SuggestionsState.Index - 1 >= 0) {
				...
				setState((State) => {
					return {
						...
						QueryDisplay:
							SuggestionsState.Suggestions[
								SuggestionsState.Suggestions.length - 1
							]?.Query ?? State.Query,
					};
				});
			}
		...
		if (e.key === "Enter") {
			if (State.FilterBookmarks.length > 0 && State.Query.length > 0) {
				...
				if (
					SuggestionsState.Index > 0 &&
					SuggestionsState.Suggestions.length > 0
				) {
					OpenLink(
						SearchEngineLinkBuilder(
							SuggestionsState.Suggestions[
								SuggestionsState.Index - 1
							].Query
						)
					...
					log(
						"Bookmark Clicked By State Empty",
						State.FilterBookmarks[0]
					);
					handleBookmarkClick(State.FilterBookmarks[0]);
				} else {
	...
	const onQueryChange = async (e: any) => {
		...
		const isExactMatch = (
			...
			return (
				...
				keywords.some((keyword) => keyword.toLowerCase() === query)
			);
		};
	...
	async function getServerBookmarks() {
		try {
			...
			let RemoveDublicatesfromLocal = State.Bookmarks.filter(
				(localBookmark) => {
					return !ProcessedBookmarks.some(
						(serverBookmark: any) =>
							serverBookmark.WebLink === localBookmark.WebLink
					);
				}
			);

			ProcessedBookmarks = ProcessedBookmarks.sort(
				() => Math.random() - 0.5
			);
			...
			setState({
				...
				Bookmarks: Array.from(
					new Set([
						...RemoveDublicatesfromLocal,
						...ProcessedBookmarks,
					])
				) as any,
				FilterBookmarks: Array.from(
					new Set([
						...RemoveDublicatesfromLocal,
						...ProcessedBookmarks,
					])
	...
	function SearchEngineLinkBuilder(query: string) {
		...
		const engineKey = State.Preferences
			.SearchEngine as string as SearchEngineKey;
		let SearchEngine =
			SearchEnginePresets[engineKey] ?? SearchEnginePresets.google;
	...
	const handleBookmarkClick = (bookmark: IBookmark, e?: any) => {
		if (
			canOpenWindowsApp(bookmark.Windows) &&
		...
		if (
			canOpenAndroidApp(bookmark.Android) &&
		...
		if (State.isFirstRun) {
			...
			if (data === null) {
				...
				setState({
					...
					FilterBookmarks: StorageData.Booksmarks.map(
						(bookmark: any) => {
							return {
								...bookmark,
								isServer: false,
	...
	return (
		<div className="Tool">
						...
						>
							<div className={`Suggestion`}>
								...
								<div className="QueryWarp">
									<h2 className="Title">
										{changeCase.upperFirst(
											"Add Bookmarks from Marketplace to Enable Query Suggestions"
										)}
									</h2>
								</div>
							</div>
				...
				{State.FilterBookmarks.length === 0 &&
					State.Bookmarks.length >= 1 && (
						<div className="Suggestions">
							{SuggestionsState.Suggestions.map(
								(item: IToolsSuggestion, index: number) => (
									<div
										key={index}
			...
			<GridContextProvider onChange={onGridChange}>
				...
				>
					{[...DefualtBookmarks, ...State.FilterBookmarks].map((item, index) => (
						<GridItem
							// className="ListItem"
							key={item.BookmarkID}
							style={{
						...
						>
							<div
								...
								onMouseDown={(
									...
									e.currentTarget.dataset.dragStartX =
										e.clientX.toString();
									e.currentTarget.dataset.dragStartY =
										e.clientY.toString();
								...
								onMouseUp={(
									...
									const dragStartX = parseInt(
										`${e.currentTarget.dataset.dragStartX}`,
										10
									);
									const dragStartY = parseInt(
										`${e.currentTarget.dataset.dragStartY}`,
										10
									);
			...
			>
				{ContextMenu &&
					(() => {
						const bookmark = State.Bookmarks.find(
							(b) => b.BookmarkID === ContextMenu?.ItemID
						);
						return (
							<div>
								{bookmark?.Windows &&
