...
function AdminDatePicker({
    ...
    useEffect(() => {
        ...
        const onDown = (e: MouseEvent) => {
            if (ref.current && !ref.current.contains(e.target as Node)) setDp((s) => ({ ...s, open: false }));
        };
        const onKey = (e: KeyboardEvent) => {
            if (e.key === "Escape") setDp((s) => ({ ...s, open: false }));
        };
    ...
    const selectDay = (d: Dayjs) => {
        ...
        setDp((s) => ({ ...s, open: false }));
    };
    const toggleOpen = () => {
        setDp((s) => ({ open: !s.open, view: "day" }));
        setCursor(parsed ?? dayjs());
    };
        ...
        <div
            ...
            <div
                ...
                style={{
                    ...
                    cursor: "pointer",
                    userSelect: "none",
                    boxShadow: dp.open ? `0 0 0 3px ${accentColor}30` : undefined,
                    border: dp.open ? `1px solid ${accentColor}` : inputStyle.border,
                ...
                <span
                    style={{
                        flex: 1,
                        fontSize: 14,
                        color: parsed ? palette.textPrimary : palette.textTertiary
                    }}>
                            ...
                            <div style={{ display: "flex", gap: 4 }}>
                                <button
                                    ...
                                    style={{
                                        ...
                                        border: "none",
                                        cursor: "pointer",
                                        background: dp.view === "month" ? `${accentColor}20` : "transparent"
                                    }}>
                                    {MONTHS[cursor.month()]}
                                </button>
                                <button
                                    ...
                                    style={{
                                        ...
                                        border: "none",
                                        cursor: "pointer",
                                        background: dp.view === "year" ? `${accentColor}20` : "transparent"
                                    }}>
                                    {cursor.year()}
                                </button>
                            </div>
...
export default function AdminCRUDPage({ title, subtitle, apiBase, idField, fields, defaultValues = {} }: AdminCRUDPageProps) {
                    ...
                    <div
                        ...
                        {tags.map((tag, i) => (
                            <span
                                ...
                                <button
                                    type="button"
                                    onClick={() => update(tags.filter((_, j) => j !== i))}
                                    style={{
                                        background: "none",
                                        border: "none",
                                        cursor: "pointer",
    ...
    const renderCellValue = (item: any, field: FieldDef) => {
        ...
        if (
            field.type === "cdn-image" ||
            (field.type === "url" && typeof val === "string" && /\.(png|jpg|jpeg|gif|webp|svg)$/i.test(val))
        ) {
            return (
                <div
                    style={{
                    ...
                    <>
                        ...
                        <motion.div
                            ...
                            <div
                                ...
                                {/* Modal footer */}
                                <div
                                    className="flex items-center justify-between gap-3 px-7 py-5 border-t flex-shrink-0"
                                    style={{ borderColor }}>
                                    <span
                                        style={{
                                            fontSize: 12,
