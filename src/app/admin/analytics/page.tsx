...
const mockAnalyticsData: AnalyticsData = {
	...
	topPages: [
		{ path: "/", views: 8543, uniqueViews: 6234, avgTimeOnPage: "3m 12s", bounceRate: 58.2 },
		{ path: "/blog", views: 5432, uniqueViews: 4321, avgTimeOnPage: "4m 45s", bounceRate: 45.6 },
		{ path: "/about", views: 3456, uniqueViews: 2876, avgTimeOnPage: "2m 23s", bounceRate: 72.1 },
		{ path: "/contact", views: 2345, uniqueViews: 1987, avgTimeOnPage: "1m 56s", bounceRate: 78.5 },
		{ path: "/services", views: 1876, uniqueViews: 1543, avgTimeOnPage: "3m 34s", bounceRate: 52.3 },
	],
	referrers: [
	...
	timeSeriesData: [
		{ date: "2024-01-15", pageViews: 1234, uniqueVisitors: 987, sessions: 876 },
		{ date: "2024-01-14", pageViews: 1543, uniqueVisitors: 1234, sessions: 1087 },
		{ date: "2024-01-13", pageViews: 1876, uniqueVisitors: 1456, sessions: 1234 },
		{ date: "2024-01-12", pageViews: 1654, uniqueVisitors: 1321, sessions: 1154 },
		{ date: "2024-01-11", pageViews: 1432, uniqueVisitors: 1176, sessions: 1032 },
		{ date: "2024-01-10", pageViews: 1765, uniqueVisitors: 1398, sessions: 1198 },
		{ date: "2024-01-09", pageViews: 1987, uniqueVisitors: 1567, sessions: 1345 },
	],
};
...
export default function AnalyticsAdminPage() {
	...
	const statsData = [
		...
		},
		{
			...
			change: { value: "-3.1%", type: "increase" as const, period: "vs last month" },
		...
		},
		{
			...
			change: { value: "+15.4%", type: "increase" as const, period: "vs last month" },
				...
				<div className="flex flex-col gap-4">
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="text-default-500">Website performance and visitor insights</p>
						</div>
						<div className="flex items-center gap-2">
							...
							<Button
								...
								onPress={() => router.push("/admin/analytics/reports")}
							>
								Generate Report
							</Button>
						</div>
					...
					<Card>
						...
						<CardBody>
							<div className="space-y-4">
								{analyticsData.timeSeriesData.slice(0, 5).map((data, index) => (
									<div key={data.date} className="flex items-center justify-between">
										...
										<div className="flex items-center gap-4">
											<div className="text-right">
												<div className="text-sm font-medium">{data.pageViews.toLocaleString()}</div>
												<div className="text-xs text-default-500">Page Views</div>
											</div>
											<div className="text-right">
												<div className="text-sm font-medium">{data.uniqueVisitors.toLocaleString()}</div>
												<div className="text-xs text-default-500">Visitors</div>
											</div>
										</div>
									</div>
								))}
								...
								{analyticsData.deviceTypes.map((device) => (
									<div key={device.type} className="space-y-2">
										...
										<Progress
											...
											className="w-full"
											color={device.type === "Desktop" ? "primary" : device.type === "Mobile" ? "success" : "warning"}
											size="sm"
										/>
									</div>
								))}
							...
							{analyticsData.countries.map((country) => (
								<div key={country.country} className="space-y-2">
									<div className="flex items-center justify-between">
										...
										<span className="text-sm text-default-500">{country.percentage}%</span>
									</div>
					...
					<CardBody>
						<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
							...
							<div className="text-center">
								...
								<div className="text-sm text-default-500">Page Views (1h)</div>
							</div>
							<div className="text-center">
								<div className="text-2xl font-bold text-warning">89</div>
								<div className="text-sm text-default-500">Sessions (1h)</div>
							</div>
							<div className="text-center">
								<div className="text-2xl font-bold text-danger">12</div>
								<div className="text-sm text-default-500">Events (1h)</div>
							</div>
						</div>
					</CardBody>
				</Card>
