...
export default function AdminBlogsPage() {
    ...
    const handleDelete = async (blog: BlogItem) => {
        ...
        try {
            const res = await fetch(`/api/admin/blogs?id=${blog._id}`, {
                method: "DELETE"
            });
            const data = await res.json();
            if (data.success) {
    ...
    const cardStyle = {
        background: isApple ? (isDark ? "rgba(28,28,32,0.7)" : "rgba(255,255,255,0.7)") : isDark ? "rgba(20,20,28,0.95)" : "#fff",
            ...
            <div className="flex gap-1.5 flex-wrap mb-6">
                {STATUS_TAB_LABELS.map((t) => {
                    ...
                    return (
                        <motion.button
                            key={t.key}
                            onClick={() => patchTable({ filter: t.key })}
                                        ...
                                        background:
                                            table.filter === t.key
                                                ...
                                                : t.color
                                                  ? t.color + "20"
                                                  : "rgba(0,0,0,0.1)",
                                        color: table.filter === t.key ? "#fff" : t.color
                                    }}>
                                    {count}
                                </span>
                            )}
                                    ...
                                    return (
                                        <tr
                                            ...
                                            <td className="px-4 py-3">
                                                <span
                                                    className="font-medium text-sm line-clamp-1 max-w-xs block"
                                                    style={{
                                                        color: palette.textPrimary
                                                    }}>
                                                    {blog.title}
            ...
            <AnimatePresence>
                {modal.approveTarget && (
                    <motion.div
                        ...
                        exit={{ opacity: 0 }}
                        onClick={(e) => {
                            if (e.target === e.currentTarget) patchModal({ approveTarget: null });
                        }}>
                        <motion.div
                                ...
                                {(["published", "unlisted", "private"] as const).map((s) => (
                                    <label
                                        ...
                                        <input
                                            type="radio"
                                            value={s}
                                            checked={modal.approveStatus === s}
                                            onChange={() => patchModal({ approveStatus: s })}
                            ...
                            <div className="flex gap-3">
                                <button
                                    onClick={() => patchModal({ approveTarget: null })}
