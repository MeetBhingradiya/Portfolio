...
export default function AdminCDNAPIKeysPage() {
    ...
    // ── State ─────────────────────────────────────────────────────────────
    // ── State ─────────────────────────────────────────────────────────────
    const [list, setList] = useState<{ keys: CDNAPIKey[]; total: number; loading: boolean; error: string; success: string; search: string; filterStatus: KeyStatus | ""; page: number }>(
        { keys: [], total: 0, loading: false, error: "", success: "", search: "", filterStatus: "", page: 1 }
    );
    ...
    // Detail panel
    const [detail, setDetail] = useState<{ selected: CDNAPIKey | null; actionLoading: boolean; revokeReason: string }>(
        { selected: null, actionLoading: false, revokeReason: "" }
    );
    const patchDetail = useCallback((p: Partial<typeof detail>) => setDetail(s => ({ ...s, ...p })), []);

    // Issue key modal
    const [issue, setIssue] = useState<{ showIssue: boolean; issuedKey: string | null; copied: boolean; loading: boolean; error: string; appQuery: string; foundApp: CDNApplication | null; appLookupError: string; notes: string; expiry: string; rl: RateLimit }>(
        { showIssue: false, issuedKey: null, copied: false, loading: false, error: "", appQuery: searchParams.get("issue") || "", foundApp: null, appLookupError: "", notes: "", expiry: "", rl: PLAN_DEFAULTS.free }
    );
    ...
    useEffect(() => {
        ...
        if (id) { patchIssue({ showIssue: true }); loadApp(id); }
    }, []);
    ...
    const fetchKeys = useCallback(async (pg = 1) => {
        ...
        } catch (e: any) { patchList({ error: e.message }); }
        finally { patchList({ loading: false }); }
    }, [list.search, list.filterStatus]);

    useEffect(() => { fetchKeys(list.page); }, [fetchKeys, list.page]);
    ...
    const loadApp = async (id: string) => {
        ...
        try {
            ...
            if (app.status !== "approved") {
                patchIssue({ appLookupError: `Application status is "${app.status}". Only approved applications can receive a key.` });
                return;
            }
            patchIssue({ foundApp: app, rl: PLAN_DEFAULTS[app.requestedPlan] || PLAN_DEFAULTS.free });
        } catch (e: any) { patchIssue({ appLookupError: e.message }); }
    };
    ...
    const handleIssue = async () => {
        ...
        try {
            ...
            fetchKeys(1);
        } catch (e: any) { patchIssue({ error: e.message }); }
        finally { patchIssue({ loading: false }); }
    };
    ...
    const doAction = async (keyId: string, action: "revoke" | "suspend" | "reinstate", reason?: string) => {
        ...
        finally { patchDetail({ actionLoading: false }); }
    };

    const copyKey = async (k: string) => { await navigator.clipboard.writeText(k); patchIssue({ copied: true }); setTimeout(() => patchIssue({ copied: false }), 2000); };
        ...
        <div className="p-6 md:p-8 space-y-6" style={{ color: palette.textPrimary }}>
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
                ...
                <div className="flex gap-2">
                    <button onClick={() => fetchKeys(list.page)} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold"
                        style={{ background: isDark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.05)", color: palette.textPrimary, border: "none", cursor: "pointer" }}>
                        <Refresh fontSize="small" />
                    </button>
                    <button onClick={() => patchIssue({ showIssue: true, issuedKey: null, foundApp: null, appQuery: "", error: "" })}
            ...
            <div className="flex gap-3 flex-wrap">
                <div className="flex-1 min-w-52 flex items-center gap-2 px-4 rounded-xl" style={{ background: isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.03)", border, borderRadius: isApple ? 12 : 14 }}>
                    ...
                    <input value={list.search} onChange={e => patchList({ search: e.target.value, page: 1 })}
                ...
                <div style={{ minWidth: 160 }}>
                    <CustomSelect
                        ...
                        onChange={v => patchList({ filterStatus: v as KeyStatus | "", page: 1 })}
                                ...
                                {list.keys.map((key, i) => {
                                    ...
                                    return (
                                        <motion.tr key={key._id}
                                            ...
                                            style={{ borderBottom: i < list.keys.length - 1 ? border : "none", cursor: "pointer" }}
                                            onClick={() => patchDetail({ selected: key, revokeReason: "" })}
                        ...
                        >
                            <div className="flex items-start justify-between mb-6">
                                <div>
                                    ...
                                    <p className="text-sm" style={{ color: palette.textSecondary }}>{detail.selected.applicantEmail}</p>
                                </div>
                                <button onClick={() => patchDetail({ selected: null })} style={{ background: "none", border: "none", cursor: "pointer", color: palette.textTertiary }}><Close /></button>
                            </div>
                            ...
                            <div className="flex items-center gap-2 mb-6 flex-wrap">
                                ...
                                <span className="px-2.5 py-1 rounded-full text-xs font-bold" style={{ background: `${PLAN_COLOR[detail.selected.plan]}18`, color: PLAN_COLOR[detail.selected.plan] }}>{detail.selected.plan}</span>
                                <code className="px-2.5 py-1 rounded-lg text-xs" style={{ background: isDark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.06)", color: palette.textSecondary }}>{detail.selected.keyPrefix}…</code>
                            </div>

                            {/* Rate limit cards */}
                            <div className="grid grid-cols-3 gap-2 mb-6">
                                {[
                                    { label: "/ min",  val: detail.selected.rateLimit.requestsPerMinute },
                                    { label: "/ hour", val: detail.selected.rateLimit.requestsPerHour },
                                    { label: "/ day",  val: detail.selected.rateLimit.requestsPerDay },
                                ].map(r => (
                            ...
                            <div className="grid grid-cols-3 gap-2 mb-6">
                                {[
                                    { label: "Requests", val: detail.selected.totalRequests },
                                    { label: "Uploads",  val: detail.selected.totalUploads },
                                    { label: "Downloads",val: detail.selected.totalDownloads },
                                ].map(r => (
                                    <div key={r.label} className="p-3 rounded-xl text-center" style={{ background: isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.03)", border }}>
                                        <div className="text-xs" style={{ color: palette.textTertiary }}>{r.label}</div>
                            ...
                            {[
                                ...
                                { label: "Issued by",  val: detail.selected.issuedBy },
                                { label: "Max file",   val: fmtBytes(detail.selected.rateLimit.maxFileSizeBytes) },
                                { label: "Upload",     val: detail.selected.rateLimit.allowUpload ? "Allowed" : "Blocked" },
                                { label: "Expires",    val: detail.selected.expiresAt ? new Date(detail.selected.expiresAt).toLocaleDateString() : "Never" },
                                { label: "Notes",      val: detail.selected.notes },
                                { label: "Last used",  val: detail.selected.lastUsedAt ? new Date(detail.selected.lastUsedAt).toLocaleString() : "—" },
                            ...
                            {detail.selected.status === "active" && (
                                <div className="mt-5">
                                    ...
                                    <input value={detail.revokeReason} onChange={e => patchDetail({ revokeReason: e.target.value })}
                            ...
                            <div className="mt-6 flex flex-col gap-2">
                                {detail.selected.status === "active" && (
                                    <>
                                        <button disabled={detail.actionLoading} onClick={() => doAction(detail.selected!.keyId, "suspend")}
                                        ...
                                        <button disabled={detail.actionLoading} onClick={() => doAction(detail.selected!.keyId, "revoke", detail.revokeReason)}
                                            className="w-full py-3 rounded-xl font-semibold flex items-center justify-center gap-2"
                                ...
                                {detail.selected.status === "suspended" && (
                                    <button disabled={detail.actionLoading} onClick={() => doAction(detail.selected!.keyId, "reinstate")}
                                        className="w-full py-3 rounded-xl font-semibold flex items-center justify-center gap-2"
                ...
                {issue.showIssue && (
                    <>
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                            ...
                            onClick={() => { if (!issue.issuedKey) patchIssue({ showIssue: false }); }} />
                        <motion.div
                        ...
                        >
                            <div className="flex items-center justify-between mb-6">
                                ...
                                {!issue.issuedKey && (
                                    <button onClick={() => patchIssue({ showIssue: false })} style={{ background: "none", border: "none", cursor: "pointer", color: palette.textTertiary }}><Close /></button>
                                )}
                            </div>
                            ...
                            {issue.issuedKey ? (
                                <div className="space-y-4">
                                    ...
                                    <div className="p-4 rounded-xl" style={{ background: "#f59e0b12", border: "1px solid #f59e0b40" }}>
                                        ...
                                        <div className="flex items-center gap-2 mt-3">
                                            ...
                                            <button onClick={() => copyKey(issue.issuedKey!)}
                                                className="p-2 rounded-xl flex-shrink-0"
                                                style={{ background: issue.copied ? "#22c55e18" : `${palette.accent}18`, color: issue.copied ? "#22c55e" : palette.accent, border: "none", cursor: "pointer" }}>
                                                {issue.copied ? <CheckCircle fontSize="small" /> : <ContentCopy fontSize="small" />}
                                            </button>
                                        </div>
                                    </div>
                                    <button onClick={() => patchIssue({ showIssue: false, issuedKey: null })}
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex gap-2">
                                            <input value={issue.appQuery} onChange={e => patchIssue({ appQuery: e.target.value })}
                                                placeholder="Paste the application MongoDB _id"
                                                style={{ ...inp(), flex: 1 }} />
                                            <button onClick={() => loadApp(issue.appQuery)}
                                        ...
                                        {issue.appLookupError && <p className="text-xs mt-1" style={{ color: "#ef4444" }}>{issue.appLookupError}</p>}
                                    </div>
                                    ...
                                    {issue.foundApp && (
                                        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                                            ...
                                            <div className="font-semibold text-sm" style={{ color: palette.textPrimary }}>{issue.foundApp.appName}</div>
                                            <div className="text-xs mt-0.5" style={{ color: palette.textSecondary }}>{issue.foundApp.applicantEmail} · Plan requested: <strong>{issue.foundApp.requestedPlan}</strong></div>
                                        </motion.div>
                                    )}
                                        ...
                                        <>
                                            <div className="grid grid-cols-3 gap-2">
                                                {[
                                                    ...
                                                    <div key={f.key}>
                                                        ...
                                                        <input type="number" min={1} value={issue.rl[f.key]}
                                                            onChange={e => patchIssue({ rl: { ...issue.rl, [f.key]: Number(e.target.value) } })}
                                                            style={{ ...inp(), width: "100%" }} />
                                                    </div>
                                                ))}
                                            </div>
                                            <div className="grid grid-cols-2 gap-2">
                                                <div>
                                                    ...
                                                    <input type="number" min={1} value={issue.rl.maxFileSizeBytes}
                                                        onChange={e => patchIssue({ rl: { ...issue.rl, maxFileSizeBytes: Number(e.target.value) } })}
                                                        style={{ ...inp(), width: "100%" }} />
                                                </div>
                                                <div>
                                                    <label className="text-xs font-semibold mb-1 block" style={{ color: palette.textSecondary }}>Expires (optional)</label>
                                                    <input type="date" value={issue.expiry} onChange={e => patchIssue({ expiry: e.target.value })}
                                                        style={{ ...inp(), width: "100%" }} />
                                                </div>
                                            </div>
                                            <div>
                                                <label className="text-xs font-semibold mb-1 block" style={{ color: palette.textSecondary }}>Admin Notes</label>
                                                <input value={issue.notes} onChange={e => patchIssue({ notes: e.target.value })}
                                    ...
                                    {issue.error && <p className="text-sm px-4 py-2 rounded-xl" style={{ background: "#ef444420", color: "#ef4444" }}>{issue.error}</p>}

                                    <button disabled={!issue.foundApp || issue.loading} onClick={handleIssue}
                                        className="w-full py-3 rounded-xl font-bold flex items-center justify-center gap-2"
                                        style={{ background: palette.accent, color: "#fff", border: "none", cursor: issue.foundApp ? "pointer" : "not-allowed", opacity: issue.foundApp ? 1 : 0.5 }}>
