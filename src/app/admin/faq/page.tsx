...
export default function AdminFAQPage() {
    ...
    }>({ faqs: [], loading: true, filter: "all", search: "", categoryFilter: "" });
    const patchList = useCallback((p: Partial<typeof list>) => setList(s => ({ ...s, ...p })), []);

    const [panel, setPanel] = useState<{ open: boolean; editId: string | null; form: ReturnType<typeof emptyForm>; saving: boolean }>(
    ...
    // ── Actions ───────────────────────────────────────────────────────────
    const openCreate = () => { patchPanel({ form: emptyForm(), editId: null, open: true }); };
    const openEdit = (f: FAQ) => {
        patchPanel({ form: { question: f.question, answer: f.answer, category: f.category, order: f.order, isPublished: f.isPublished }, editId: f._id, open: true });
    };
    ...
    return (
        <div className="p-6 max-w-5xl mx-auto">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm" style={{ color: palette.textSecondary }}>
                            {counts.all} entries · {counts.published} published · <span style={{ color: "#FF9500" }}>{counts.draft} drafts</span>
                        </p>
                    </div>
                </div>
                <motion.button
            ...
            <div className="flex flex-wrap items-center gap-3 mb-4">
                ...
                {/* Search */}
                <div className="flex-1 min-w-48 flex items-center gap-2 px-3 py-2 rounded-xl" style={{ background: isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.05)", border }}>
            ...
            {list.loading ? (
                <div className="space-y-2">
                    {[1, 2, 3, 4].map(i => <div key={i} className="h-16 rounded-2xl animate-pulse" style={{ background: isDark ? "rgba(255,255,255,0.04)" : "rgba(0,0,0,0.04)" }} />)}
                </div>
            ) : (
                <div className="space-y-2">
                    <AnimatePresence>
                        {visible.map(f => (
                            ...
                            >
                                {/* Draft indicator bar */}
                                {!f.isPublished && (
                                    <div className="w-1 h-10 rounded-full shrink-0" style={{ background: "#FF9500" }} />
                                )}
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                                        ...
                                        <span className="text-xs" style={{ color: palette.textTertiary }}>#{f.order}</span>
                                    </div>
                                    <p className="font-bold text-sm truncate" style={{ color: palette.textPrimary }}>{f.question}</p>
                                    <p className="text-xs truncate mt-0.5" style={{ color: palette.textSecondary }}>{f.answer}</p>
                                </div>

                                <div className="flex items-center gap-1 text-xs shrink-0" style={{ color: palette.textTertiary }}>
                                    <span>👍 {f.helpful}</span>
                                ...
                                <div className="flex gap-1 shrink-0">
                                    <motion.button
                                        ...
                                        style={{ color: f.isPublished ? "#34C759" : "#FF9500" }}
                                    >
                    ...
                    {visible.length === 0 && (
                        <div className="text-center py-16" style={{ color: palette.textSecondary }}>
                            {list.filter === "draft" ? "No draft entries." : list.filter === "published" ? "No published FAQ entries." : "No FAQ entries yet. Create one!"}
                        </div>
                    )}
                </div>
            )}
                    ...
                    <>
                        ...
                        >
                            <div className="flex items-center justify-between px-6 py-5 border-b" style={{ borderColor: isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.08)" }}>
                                <div>
                                    <h2 className="font-black text-lg" style={{ color: palette.textPrimary }}>{panel.editId ? "Edit FAQ" : "New FAQ"}</h2>
                                    <p className="text-xs mt-0.5" style={{ color: panel.form.isPublished ? "#34C759" : "#FF9500" }}>
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-4">
                                <input
                                    ...
                                    onChange={e => patchPanel({ form: { ...panel.form, question: e.target.value } })}
                                />
                                <textarea
                                    ...
                                    onChange={e => patchPanel({ form: { ...panel.form, answer: e.target.value } })}
                                />
                                <div className="grid grid-cols-2 gap-3">
                                    <CustomSelect
                                        value={panel.form.category}
                                        onChange={v => patchPanel({ form: { ...panel.form, category: v } })}
                                    ...
                                    <input
                                        ...
                                        onChange={e => patchPanel({ form: { ...panel.form, order: Number(e.target.value) } })}
                                    />
                                </div>

                                {/* Publish toggle */}
                                <div className="flex items-center justify-between px-4 py-3 rounded-xl"
                                    style={{ background: panel.form.isPublished ? "rgba(52,199,89,0.08)" : "rgba(255,149,0,0.08)", border: `1px solid ${panel.form.isPublished ? "rgba(52,199,89,0.25)" : "rgba(255,149,0,0.25)"}` }}>
                                    <div>
                                    ...
                                    <div
                                        ...
                                        style={{ background: panel.form.isPublished ? "#34C759" : isDark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.1)" }}
                                        onClick={() => patchPanel({ form: { ...panel.form, isPublished: !panel.form.isPublished } })}
                                    >
                                        <div className="absolute top-1 w-4 h-4 rounded-full bg-white transition-transform shadow"
                                            style={{ transform: `translateX(${panel.form.isPublished ? 26 : 4}px)` }} />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="px-6 pb-6 pt-3 border-t" style={{ borderColor: isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.08)" }}>
                                <motion.button
                                    ...
                                    style={{ background: palette.accent, color: "#fff", opacity: !panel.form.question.trim() ? 0.5 : 1 }}
                                >
