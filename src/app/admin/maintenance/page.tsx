...
export default function AdminMaintenancePage() {
    ...
    const fetchStatus = useCallback(async () => {
        ...
        try {
            ...
            patch({
                status: {
                    type: "error",
                    msg: "Failed to load maintenance status."
                }
            });
        } finally {
            patch({ loading: false });
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (data.success) {
                patch({
                    status: { type: "success", msg: data.message },
    ...
    return (
        <div className="p-6 max-w-2xl mx-auto flex flex-col gap-6">
            ...
            {!state.loading && (
                <div
                    ...
                    <span
                        ...
                        style={{
                            color: state.mode ? (isDark ? "#fdba74" : "#9a3412") : isDark ? "#86efac" : "#15803d"
                        }}>
            ...
            <motion.div
                ...
                <div className="flex items-center justify-between">
                    ...
                    <motion.button
                        ...
                        style={{
                            background: state.mode ? palette.accent : isDark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.15)"
                        }}
                        whileTap={{ scale: 0.94 }}>
                        <motion.span
            ...
            <div
                ...
                <strong>How it works:</strong> When enabled, all pages redirect non-admin visitors to{" "}
                <code
                    className="px-1 py-0.5 rounded text-xs"
                    style={{ background: "rgba(59,130,246,0.15)" }}>
                    /maintenance
                </code>{" "}
                and all API routes return a 503 response. Admin users with a valid session bypass this automatically after visiting this
                page.
            </div>
        </div>
    );
