...
const folders = ["all", "banners", "videos", "audio", "documents", "icons", "logos"];
...
export default function MediaPage() {
	...
	const statsData = [
		...
		{
			title: "Total Size",
			value: formatFileSize(mediaFiles.reduce((acc, file) => acc + file.size, 0)),
	...
	function getFileIcon(type: string) {
		switch (type) {
			...
			case "video": return <VideoLibrary className="w-5 h-5" />;
			case "audio": return <AudioFile className="w-5 h-5" />;
			default: return <InsertDriveFile className="w-5 h-5" />;
		}
	}
	...
	const handleFileUpload = useCallback(async (files: FileList) => {
		...
		for (let i = 0; i < files.length; i++) {
			...
			const newMediaFile: MediaFile = {
				...
				uploadDate: new Date(),
				uploader: currentAccount?.FName + " " + currentAccount?.LName || "Unknown",
				tags: [],
				downloads: 0,
				folder: "uploads",
			};
			...
			<div className="p-6 space-y-6">
				...
				<div className="flex flex-col gap-4">
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="text-default-500">Manage your files, images, videos, and documents</p>
						</div>
					...
					<CardBody>
						<div className="flex flex-col sm:flex-row gap-4">
							...
							<Select
								...
								onSelectionChange={(keys) => setSelectedFolder(Array.from(keys)[0] as string)}
								className="w-40"
							>
								{folders.map(folder => (
										...
										<TableCell>
											<div className="flex items-center gap-3">
												...
												<div>
													...
													<p className="text-sm text-default-500">{file.folder}</p>
												</div>
											</div>
										</TableCell>
										<TableCell>
										...
										<TableCell>
											<Dropdown>
												<DropdownTrigger>
													...
													<DropdownItem key="share" startContent={<Share className="w-4 h-4" />}>
														Share
													</DropdownItem>
						...
						<ModalBody>
							<div className="space-y-4">
								...
								>
									...
									<p className="text-default-500">Support for images, videos, audio, and documents</p>
									<input
										type="file"
										multiple
										className="hidden"
				...
				<Modal isOpen={isDeleteOpen} onClose={onDeleteClose}>
					<ModalContent>
						...
						<ModalBody>
							<p>Are you sure you want to delete {selectedFiles.size} file(s)? This action cannot be undone.</p>
						</ModalBody>
						<ModalFooter>
