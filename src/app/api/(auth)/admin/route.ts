...
async function getAdminDashboard(req: AuthenticatedRequest): Promise<NextResponse> {
    try {
        await dbConnect();

        // Get user statistics
        const totalUsers = await Users_Model.countDocuments({ isDeleted: false });
        ...
        const activeSessions = await Sessions_Model.countDocuments({ isActive: true });
        ...
        try {
            ...
            ticketStats.open = await Tickets_Model.countDocuments({ Status: 'Open' });
            ticketStats.closed = await Tickets_Model.countDocuments({ Status: 'Closed' });
            ticketStats.pending = await Tickets_Model.countDocuments({ Status: 'Pending' });
        } catch (error) {
...
async function getUserManagement(req: AuthenticatedRequest): Promise<NextResponse> {
    try {
        ...
        return NextResponse.json({
            ...
            Data: {
                users: users.map(user => ({
                    ...
                    isEmailVerified: user.Emails.some(email => email.isVerified),
...
async function updateUserStatus(req: AuthenticatedRequest): Promise<NextResponse> {
    try {
        ...
        // Validate action
        const validActions = ['lock', 'unlock', 'suspend', 'unsuspend', 'delete', 'makeAdmin', 'removeAdmin'];
        ...
        log(`Admin ${req.user!.UserID} ${actionDescription} user ${Request.userID}`);
