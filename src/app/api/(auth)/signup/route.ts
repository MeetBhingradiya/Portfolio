...
import { Encrypt, generateSalt, generateRounds, generateSecret } from "@Utils/Crypto";
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (!emailSent) {
            log(`Failed to send verification email to ${Body.email} for user ${newUser.UserID}`);
            // Note: User is created but email failed - we'll still return success
            // User can request resend OTP later
        }
        
        // ? Create Session and RSA Key Pairs
        // TODO: Implement session creation for new user        // ? Return Response
        return NextResponse.json({
            Status: 1,
            Message: emailSent 
