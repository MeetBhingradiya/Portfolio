...
function detectAutomationBot(req: NextRequest): { isBot: boolean; reason: string } {
    ...
    for (const [category, signatures] of Object.entries(BOT_SIGNATURES)) {
        for (const signature of signatures) {
            if (userAgent.includes(signature)) {
                return { isBot: true, reason: `Automation tool detected: ${signature} (${category})` };
            }
        }
    }
    ...
    for (const header of AUTOMATION_HEADERS) {
        if (req.headers.has(header)) {
            return { isBot: true, reason: `Automation header detected: ${header}` };
        }
    }
    ...
    for (const header of REQUIRED_HEADERS) {
        if (!req.headers.has(header)) {
            return { isBot: true, reason: `Missing required header: ${header}` };
        }
    }
    ...
    if (secUa) {
        ...
        if (secUa.includes('headlesschrome') || secUa.includes('not a;brand')) {
            return { isBot: true, reason: 'Headless browser detected in Sec-CH-UA' };
        }
    }
    
    // 6. Check Accept-Language patterns
    if (!acceptLanguage || acceptLanguage === 'en-US' || acceptLanguage === 'en') {
    ...
    for (const pattern of suspiciousUaPatterns) {
        if (pattern.test(userAgent)) {
            return { isBot: true, reason: 'Suspicious User-Agent pattern detected' };
        }
    }
...
function analyzeRequestFingerprint(req: NextRequest): { suspicious: boolean; reason: string } {
    ...
    if (headerCount < 10 || headerCount > 25) {
        return { suspicious: true, reason: `Too many or few headers: ${headerCount}` };
    }
...
export async function POST(req: NextRequest) {
    ...
    if (botDetection.isBot) {
        return ControllerResponseMap({
            ...
            StatusNumber: 403,
            Debug: Config.Environment === 'development' ? { reason: botDetection.reason } : undefined
        });
    }
    ...
    if (parsedUA.isBot) {
        return ControllerResponseMap({
            ...
            StatusNumber: 403,
            Debug: Config.Environment === 'development' ? { botType: parsedUA.isBot } : undefined
        });
    }

    // 3. Browser and Platform Validation
    if (!WhiteListedBrowsers.includes(changeCase.upperFirst(parsedUA.browser) as any)) {
        return ControllerResponseMap({
            Status: 0,
            Message: 'Unsupported browser',
            StatusCode: 'UNSUPPORTED_BROWSER',
            StatusNumber: 403,
            Debug: Config.Environment === 'development' ? { browser: parsedUA.browser } : undefined
        });
    }

    if (!WhiteListedPlatforms.includes(changeCase.upperFirst(secUaPlatform.replace(/"/g, '')) as any)) {
        return ControllerResponseMap({
            Status: 0,
            Message: 'Unsupported Platform',
            StatusCode: 'UNSUPPORTED_PLATFORM',
            StatusNumber: 403,
            Debug: Config.Environment === 'development' ? { platform: secUaPlatform } : undefined
        });
    }
