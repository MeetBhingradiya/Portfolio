/**
 * Admin — SSO App Access item API
 * PATCH  /api/admin/sso-access/[id]   update (enable/disable, note)
 * DELETE /api/admin/sso-access/[id]   remove
 */
import { NextRequest, NextResponse } from "next/server";
import { requirePermission, permissionError } from "@Library/adminApiMiddleware";
import dbConnect from "@Utils/dbConnect";
import { SSOAppAccess } from "@Models/SSO";
...
export async function PATCH(req: NextRequest, context: { params: Promise<{ id: string }> }) {
    try {
        ...
        await dbConnect();
        const entry = await SSOAppAccess.findById(id);
        if (!entry) {
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ id: string }> }) {
    try {
        ...
        await dbConnect();
        const result = await SSOAppAccess.findByIdAndDelete(id);
        if (!result) {
            return NextResponse.json({ success: false, error: "Not found" }, { status: 404 });
        }
