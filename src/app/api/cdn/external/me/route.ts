    ...
    return NextResponse.json({
        ...
        usage: {
            ...
            currentWindows: {
                minute: {
                    used: used.minute,
                    limit: key.rateLimit.requestsPerMinute,
                    remaining: Math.max(0, key.rateLimit.requestsPerMinute - used.minute)
                },
                hour: {
                    used: used.hour,
                    limit: key.rateLimit.requestsPerHour,
                    remaining: Math.max(0, key.rateLimit.requestsPerHour - used.hour)
                },
                day: {
                    used: used.day,
                    limit: key.rateLimit.requestsPerDay,
                    remaining: Math.max(0, key.rateLimit.requestsPerDay - used.day)
                }
            }
        }
    });
