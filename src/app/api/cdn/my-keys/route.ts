...
export async function GET(req: NextRequest) {
    try {
        ...
        const keys = await CDNAPIKey.find({ applicantEmail: email }).select("-keyHash").sort({ createdAt: -1 }).lean();

        // Fetch corresponding applications for context
        const applicationIds = [...new Set(keys.map((k) => k.applicationId))];
        const applications = await CDNApplication.find({
        ...
        const windows = activeKeyIds.length ? await CDNRateWindow.find({ keyId: { $in: activeKeyIds } }).lean() : [];
