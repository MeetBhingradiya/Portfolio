...
export async function GET(req: NextRequest) {
    try {
        ...
        const reminders = await ProductivityReminder.find(query).sort({ ScheduledAt: 1, Priority: -1 }).lean();
...
export async function POST(req: NextRequest) {
    try {
        ...
        await UserProductivityStats.updateOne({ UserID: user.userId }, { $inc: { TotalRemindersCreated: 1 } }, { upsert: true });
