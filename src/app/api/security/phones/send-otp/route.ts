...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!E164_PHONE_REGEX.test(phoneNumber)) {
            return NextResponse.json({ success: false, error: "Invalid phone number format. Use E.164 format." }, { status: 400 });
        }

        const policies = await getPhonePolicies();
        await dbConnect();

        const existingForUser = await UserPhone.findOne({ userId: session.user.id, phoneNumber }).lean();
        if (existingForUser?.verified) {
            return NextResponse.json({ success: false, error: "This phone number is already verified on your account." }, { status: 409 });
        }

        const countForUser = await UserPhone.countDocuments({ userId: session.user.id, verified: true });
        if (countForUser >= policies.maxPhonesPerAccount) {
            return NextResponse.json(
                { success: false, error: `Phone limit reached. Max ${policies.maxPhonesPerAccount} numbers per account.` },
                { status: 400 }
            );
        }
        ...
        if (countForPhone >= policies.maxAccountsPerPhone) {
            return NextResponse.json(
                { success: false, error: `This phone is already linked to ${policies.maxAccountsPerPhone} accounts (max).` },
                { status: 400 }
            );
        }
        ...
        await PhoneOtpChallenge.deleteMany({ userId: session.user.id, phoneNumber });
