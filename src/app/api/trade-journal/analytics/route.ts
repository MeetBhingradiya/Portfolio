    ...
    try {
        ...
        const query: Record<string, any> = {
            ...
            $or: [
                ...
                {
                    Result: {
                        $in: [TradeResult.WIN, TradeResult.LOSS, TradeResult.BREAKEVEN]
                    }
                }
            ]
        };
        ...
        const avgChargesAgg = await TradeJournal.aggregate([
            { $match: chargeBaseMatch },
            {
                $project: {
                    charges: {
                        $add: [{ $ifNull: ["$Brokerage", 0] }, { $ifNull: ["$Taxes", 0] }]
                    }
                }
            },
            {
        ...
        const dailyCapitalRecords: any[] = await DailyCapital.find(dailyCapitalQuery).sort({ Date: 1 }).lean();
        ...
        if (dailyReturns.length >= 2) {
            ...
            sharpeRatio = stdDev > 0 ? parseFloat((((avg - RISK_FREE_DAILY) / stdDev) * Math.sqrt(252)).toFixed(3)) : 0;
        }
        ...
        for (let i = 0; i < trades.length; i++) {
            ...
            equityCurve.push({
                date: new Date(trades[i].Date as Date).toISOString().slice(0, 10),
        ...
        const avgActualRR = tradesWithRR.length ? tradesWithRR.reduce((s, t) => s + t.ActualRR, 0) / tradesWithRR.length : 0;
        ...
        const byInstrument = Object.entries(instMap)
            .map(([instrument, v]) => ({
                instrument,
                trades: v.trades,
        ...
        const byEmotion = Object.entries(emotMap)
            .map(([emotion, v]) => ({
                emotion,
                trades: v.trades,
                pnl: parseFloat(v.pnl.toFixed(2)),
        ...
        const uniqueTradingDays = new Set(trades.map((t) => new Date(t.Date as Date).toISOString().slice(0, 10))).size;
...
function emptyAnalytics(charges?: { avgCharges?: number; todayCharges?: number }) {
    return {
        summary: {
            ...
            highestPnL: 0,
            lowestPnL: 0,
            avgCharges: charges?.avgCharges ?? 0,
            todayCharges: charges?.todayCharges ?? 0,
            sharpeRatio: 0,
            dailyCapitalDays: 0,
            edgeScore: 0,
            edgeScoreMax: 8
