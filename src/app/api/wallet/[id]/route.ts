...
async function applyBalance(
    ...
    if (type === "DEBIT" && resolvedFrom) {
        await WalletAsset.updateOne({ AssetID: resolvedFrom, UserID: userId }, { $inc: { Balance: -amount * mult } });
    } else if (type === "CREDIT" && resolvedTo) {
        await WalletAsset.updateOne({ AssetID: resolvedTo, UserID: userId }, { $inc: { Balance: amount * mult } });
    } else if (type === "TRANSFER") {
        await Promise.all([
            resolvedFrom
                ? WalletAsset.updateOne({ AssetID: resolvedFrom, UserID: userId }, { $inc: { Balance: -amount * mult } })
                : Promise.resolve(),
            resolvedTo
                ? WalletAsset.updateOne({ AssetID: resolvedTo, UserID: userId }, { $inc: { Balance: amount * mult } })
                : Promise.resolve()
        ]);
    }
}
...
export async function PUT(req: NextRequest, ctx: Ctx) {
    try {
        ...
        const oldTxn = (await WalletTransaction.findOne({
            TransactionID: id,
            UserID: user.userId
        }).lean()) as any;
        ...
        // Apply new balance
        await applyBalance(txn.Type, txn.FromAssetID, txn.ToAssetID, txn.Amount, user.userId, false);
...
export async function DELETE(_req: NextRequest, ctx: Ctx) {
    try {
        ...
        const { id } = await ctx.params;
        const txn = (await WalletTransaction.findOneAndDelete({
            TransactionID: id,
            UserID: user.userId
        }).lean()) as any;
        if (!txn) return NextResponse.json({ success: false, error: "Not found" }, { status: 404 });
