...
function SignInForm() {
	...
	const { AddAccount, Accounts, ActiveAccount } = useAccountSwitcher();	const [state, setState] = React.useState<SigninState>({
		username: "",
		password: "",
		isLoading: false,
		showPassword: false,
		...
		const checkPasskeySupport = async () => {
			try {
				...
				if (isSupported) {
					...
					setState(prev => ({ ...prev, isPasskeySupported: available }));
				}
			} catch (error) {
	...
	const handleSignin = async () => {
		...
		try {
			...
			if (response.data.Status === 1) {
				...
				const maxAge = state.rememberMe ? 30 * 24 * 60 * 60 : 24 * 60 * 60; // 30 days or 1 day
				...
				let errorMessage = response.data.Message || "Signin failed";				let accountStatus: "active" | "suspended" | "blocked" | null = null;

				// Check for specific status codes
				if (response.data.StatusCode === 403) {
					if (response.data.Message.toLowerCase().includes('suspended')) {
						accountStatus = 'suspended';
						errorMessage = "Your account has been suspended. Please contact support for assistance.";
					} else if (response.data.Message.toLowerCase().includes('blocked')) {
						accountStatus = 'blocked';
						errorMessage = "Your account has been blocked due to security concerns. Please contact support.";
					}
				}
			...
			console.error("Signin error:", error);			let errorMessage = "An unexpected error occurred";
	...
	return (
		<div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50 dark:from-gray-900 dark:via-gray-800 dark:to-blue-900 relative overflow-hidden">
			...
			<Modal
				...
				closeButton={false}
				classNames={{
					backdrop: "bg-gradient-to-t from-zinc-900/50 to-zinc-900/50 backdrop-blur-sm",
						...
						<ModalBody className="flex flex-col gap-6 p-8 bg-white/50 dark:bg-gray-900/50 backdrop-blur-sm">
								...
								{state.error && (
									...
									>
										<Alert
											...
											title={
												state.accountStatus === 'suspended' ? "Account Suspended" :
												state.accountStatus === 'blocked' ? "Account Blocked" :
												"Sign In Error"
											}
											color={getErrorColor()}
											variant="bordered"
							...
							{state.isPasskeySupported && (
								...
								>
									<div className="flex items-center justify-between">
										...
										<Button
											size="sm"
											variant={state.usePasskey ? "solid" : "bordered"}
											color="primary"
											onPress={() => setState(prev => ({ ...prev, usePasskey: !prev.usePasskey }))}
											startContent={state.usePasskey ? <FaceRetouchingNatural /> : <Fingerprint />}
										>
											{state.usePasskey ? "Enabled" : "Enable"}
										</Button>
									</div>
								</motion.div>
							)}
							...
							>
								<Input
									...
									classNames={{
										input: "text-base",
										inputWrapper: "border-gray-300 dark:border-gray-600 hover:border-blue-400 dark:hover:border-blue-500 focus-within:border-blue-500 dark:focus-within:border-blue-400 bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm",
									}}
							...
							>
								<Button
									...
									isDisabled={
										state.isLoading ||
										!state.username.trim() ||
										(!state.usePasskey && !state.password.trim())
									}
									...
									startContent={
										!state.isLoading && (
											state.usePasskey ? <Fingerprint /> : <PersonAdd />
										)
									}
									spinner={<CircularProgress size="sm" color="primary" />}
								>
									{state.isLoading 
										? (state.usePasskey ? "Authenticating..." : "Signing In...") 
										: (state.usePasskey ? "Sign In with Passkey" : "Sign In")
									}
								</Button>
							</motion.div>
						</ModalFooter>
				...
				<Card className="w-[400px] bg-white/10 dark:bg-gray-900/10 backdrop-blur-md border border-white/20">
					<CardBody className="flex flex-col items-center gap-6 p-8">
						...
						<div className="text-center space-y-2">
							...
							<p className="text-gray-600 dark:text-gray-300">Loading sign in page...</p>
						</div>
