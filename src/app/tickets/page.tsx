...
function TicketStatusPage() {
        ...
        switch (status) {
            ...
            case 'in-progress':
                return {
                    ...
                    label: 'In Progress',
                    description: 'Your ticket is currently being worked on by our team'
                };
            case 'resolved':
                return {
                    ...
                    label: 'Resolved',
                    description: 'Your ticket has been resolved. Please check the responses below'
                };
            case 'closed':
                return {
                    icon: <CheckCircle className="text-lg" />,
    ...
    const getProjectTypeIcon = (type: string) => {
        switch (type) {
            case 'staff-engineer': return <Business className="text-sm" />;
            case 'consulting': return <Security className="text-sm" />;
            case 'collaboration': return <Star className="text-sm" />;
            case 'freelance': return <Message className="text-sm" />;
            case 'mentorship': return <Support className="text-sm" />;
            default: return <Info className="text-sm" />;
        }
    };
                ...
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    >
                        <h1 className="text-4xl sm:text-5xl font-bold text-gray-900 dark:text-white mb-4">
                            Ticket <span className="bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Status</span>
                        </h1>
                        <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
                            Track your support ticket and see real-time updates on your request. 
                            Get instant access to responses and status changes.
                        </p>
                    </motion.div>
                                ...
                                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-8">
                                    <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
                                        ...
                                        <div className="flex flex-wrap gap-3">
                                            {/* Status Badge */}
                                            <div className={`inline-flex items-center space-x-2 px-4 py-2 rounded-full text-sm font-medium ${getStatusConfig(ticket.status).bgColor} ${getStatusConfig(ticket.status).borderColor} border`}>
                                                {getStatusConfig(ticket.status).icon}
                                                <span className={getStatusConfig(ticket.status).textColor}>
                                                    {getStatusConfig(ticket.status).label}
                                                </span>
                                            </div>
                                            
                                            {/* Priority Badge */}
                                            <div className={`inline-flex items-center space-x-2 px-4 py-2 rounded-full text-sm font-medium ${getPriorityConfig(ticket.priority).bgColor}`}>
                                                {getPriorityConfig(ticket.priority).icon}
                                                <span className={getPriorityConfig(ticket.priority).color}>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                                        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                                            <div className="flex items-center space-x-2 mb-2">
                                                ...
                                                <span className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Ticket ID</span>
                                            </div>
                                            <span className="text-sm font-mono text-gray-900 dark:text-white">{ticket.id}</span>
                                        </div>
                                        
                                        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                                            <div className="flex items-center space-x-2 mb-2">
                                                {getProjectTypeIcon(ticket.projectType)}
                                                <span className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Project Type</span>
                                            </div>
                                            <span className="text-sm text-gray-900 dark:text-white capitalize">{ticket.projectType.replace('-', ' ')}</span>
                                        </div>
                                        ...
                                        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                                            ...
                                            <span className="text-sm text-gray-900 dark:text-white">{formatDate(ticket.createdAt)}</span>
                                        </div>
                                        ...
                                        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                                            ...
                                            <span className="text-sm text-gray-900 dark:text-white">{formatDate(ticket.updatedAt)}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                            ...
                                            {ticket.responses && ticket.responses.length > 0 ? (
                                                ...
                                                <div className="text-center py-8">
                                                    ...
                                                    <p className="text-gray-600 dark:text-gray-400">
                                                        We&apos;ll update you as soon as there&apos;s a response to your ticket.
                                                    </p>
                                                </div>
                                            )}
                                        </div>
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-6">
                                            ...
                                            <div className={`p-4 rounded-lg ${getStatusConfig(ticket.status).bgColor} ${getStatusConfig(ticket.status).borderColor} border`}>
                                                <div className="flex items-center justify-between mb-2">
                                                    <span className={`font-semibold ${getStatusConfig(ticket.status).textColor}`}>
                                                        {getStatusConfig(ticket.status).label}
                                                    </span>
                                                    <span className={`text-xs px-2 py-1 rounded-full ${getPriorityConfig(ticket.priority).bgColor} ${getPriorityConfig(ticket.priority).color}`}>
                                                        {ticket.priority}
                                                    </span>
                                                </div>
                                                <p className={`text-sm ${getStatusConfig(ticket.status).textColor} opacity-90`}>
                                        ...
                                        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-6">
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    <span className="text-sm text-gray-600 dark:text-gray-400">1-2 days</span>
                                                </div>
                                                <div className="flex justify-between items-center">
                                        ...
                                        <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 rounded-xl border border-blue-200 dark:border-blue-800 p-6">
                                            ...
                                            <div className="space-y-3 text-sm text-gray-600 dark:text-gray-300">
                                                <p><strong>Can&apos;t find your ticket?</strong></p>
