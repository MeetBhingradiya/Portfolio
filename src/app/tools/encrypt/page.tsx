...
import { Lock, LockOpen, ContentCopy, Check, SwapVert, History, Delete, Key } from "@mui/icons-material";
    ...
    const process = useCallback(async () => {
        ...
        try {
            ...
            if (direction === "encrypt") {
                switch (algorithm) {
                    case "AES":
                        result = CryptoJS.AES.encrypt(input, secret).toString();
                        break;
                    case "DES":
                        result = CryptoJS.DES.encrypt(input, secret).toString();
                        break;
                    case "TripleDES":
                        result = CryptoJS.TripleDES.encrypt(input, secret).toString();
                        break;
                    case "Rabbit":
                        result = CryptoJS.Rabbit.encrypt(input, secret).toString();
                        break;
                    case "RC4":
                        result = CryptoJS.RC4.encrypt(input, secret).toString();
                        break;
                }
            } else {
                let bytes;
                switch (algorithm) {
                    case "AES":
                        bytes = CryptoJS.AES.decrypt(input, secret);
                        break;
                    case "DES":
                        bytes = CryptoJS.DES.decrypt(input, secret);
                        break;
                    case "TripleDES":
                        bytes = CryptoJS.TripleDES.decrypt(input, secret);
                        break;
                    case "Rabbit":
                        bytes = CryptoJS.Rabbit.decrypt(input, secret);
                        break;
                    case "RC4":
                        bytes = CryptoJS.RC4.decrypt(input, secret);
                        break;
                }
            ...
            <div className="space-y-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {ALGORITHMS.map((a) => (
                            <motion.button
                                ...
                                style={{
                                    background:
                                        algorithm === a.alg ? palette.accent : isDark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.04)",
                        ...
                        <div className="space-y-1">
                            <div className="flex justify-between">
                                ...
                                <span
                                    className="text-[10px] font-bold"
                                    style={{
                                        color: STRENGTH_COLORS[keyStrength]
                                    }}>
                                    {STRENGTH_LABELS[keyStrength]}
                                </span>
                            </div>
                ...
                <motion.button
                    ...
                    style={{
                        background: palette.accent,
                        color: "#fff",
                        opacity: processing || !input || !secret ? 0.5 : 1
                    }}
                    whileTap={{ scale: 0.98 }}>
