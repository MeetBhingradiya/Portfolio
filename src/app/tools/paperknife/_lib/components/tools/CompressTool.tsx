    ...
    const ActionButton = () => (
        <button
            ...
            {isProcessing ? (
                ...
                <>
                    Compress {files.length > 1 ? `${files.length} Files` : "PDF"} <ArrowRight size={18} />
                </>
            )}
        </button>
    );
                ...
                <div className="space-y-6 animate-in fade-in duration-500">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    {f.isLocked ? (
                                        <div className="flex gap-1 mt-1">
                                            <input
                                                type="password"
                                                placeholder="Locked..."
                                                className="flex-1 bg-gray-50 dark:bg-black text-[10px] p-1.5 rounded-lg outline-none w-full border border-gray-100 dark:border-zinc-800 focus:border-rose-500"
                                                onKeyDown={(e) => {
                        ...
                        <button
                            ...
                            className="border-2 border-dashed border-gray-100 dark:border-zinc-800 rounded-[1.5rem] p-4 text-gray-400 flex flex-col items-center justify-center gap-1 hover:border-rose-500 hover:text-rose-500 transition-all">
                            <Plus size={20} />
                    ...
                    <div className="bg-white dark:bg-zinc-900 p-8 rounded-[2rem] border border-gray-100 dark:border-white/5 shadow-sm">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            {[
                                ...
                                <button
                                    ...
                                    className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-1 ${quality === lvl.id ? "border-rose-500 bg-rose-50/50 dark:bg-rose-900/10" : "border-gray-100 dark:border-white/5"}`}>
                                    <span
                                        className={`font-black uppercase text-[9px] text-center leading-tight ${quality === lvl.id ? "text-rose-500" : "text-gray-400"}`}>
                                        {lvl.label}
                                    </span>
                        ...
                        <div className="mt-6 p-6 bg-gray-50 dark:bg-black rounded-2xl border border-gray-100 dark:border-white/5">
                            ...
                            <p className="text-xs text-gray-500 dark:text-zinc-400 leading-relaxed">
                                {quality === "high" && (
                                    <>
                                        <strong>High Quality:</strong> Retains maximum text clarity and image resolution. Best for official
                                        documents and high-fidelity reports. Expected reduction:{" "}
                                        <span className="text-rose-500 font-bold">10-30%</span>.
                                    </>
                                )}
                                {quality === "medium" && (
                                    <>
                                        <strong>Standard:</strong> Balanced optimization for everyday sharing and email attachments. The
                                        perfect middle ground for most users. Expected reduction:{" "}
                                        <span className="text-rose-500 font-bold">40-60%</span>.
                                    </>
                                )}
                                {quality === "low" && (
                                    <>
                                        <strong>Smallest Size:</strong> Aggressive downsampling for the lowest possible file size. Ideal for
                                        quick mobile viewing or meeting strict upload limits. Expected reduction:{" "}
                                        <span className="text-rose-500 font-bold">70-90%</span>.
                                    </>
                                )}
                            </p>
                        </div>
                ...
                <div className="space-y-6 animate-in zoom-in duration-300">
                    {objectUrl && files.length > 1 && (
                        <button
                            ...
                            <div className="w-16 h-16 bg-rose-500 rounded-full flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform shadow-lg">
                                <Download
