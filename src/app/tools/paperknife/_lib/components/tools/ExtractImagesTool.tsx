...
import { Image as ImageIcon, Lock, Loader2, X, Sparkles } from "@/app/tools/paperknife/_lib/muiLucide";
...
export default function ExtractImagesTool() {
        ...
        try {
            ...
            for (let i = 1; i <= pdfData.pageCount; i++) {
                ...
                for (let j = 0; j < operatorList.fnArray.length; j++) {
                    ...
                    if (fn === 85 || fn === 82 || fn === 92 || fn === 87) {
                        // internal codes for image ops in some pdfjs versions, but let's be safer
    ...
    const ActionButton = () => (
        <button
            ...
            {isProcessing ? (
                ...
                <>
                    <Sparkles size={18} /> Extract Raw Images
                </>
            )}
        </button>
    );
    ...
    return (
        <NativeToolLayout
            ...
            description="Find and save all original images embedded inside the PDF."
                                ...
                                <div className="space-y-4">
                                    <div className="text-center py-2 px-4 bg-gray-50 dark:bg-black rounded-2xl border border-gray-100 dark:border-white/5">
                                        <p className="text-xs text-gray-500 dark:text-zinc-400 leading-relaxed font-medium">
                                            This tool scans every page and recovers high-quality source images. Perfect for saving photos
                                            from documents.
                                        </p>
                                    </div>
                                    <div className="p-4 bg-amber-50 dark:bg-amber-900/10 rounded-xl border border-amber-100 dark:border-amber-900/20 text-center">
                                        <p className="text-[10px] text-amber-700 dark:text-amber-400 font-bold uppercase tracking-widest">
                                            Note: If no images are detected, the file may have been compressed using rasterization (printing
                                            to PDF).
                                        </p>
                                    </div>
                                </div>
