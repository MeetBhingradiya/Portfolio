...
import { Plus, X, Loader2, GripVertical, Upload, ArrowRight } from "@/app/tools/paperknife/_lib/muiLucide";
...
function SortableImageItem({ id, img, onRemove }: { id: string; img: ImageFile; onRemove: (id: string) => void }) {
    ...
    return (
        <div
            ref={setNodeRef}
            style={style}
            className={`flex items-center gap-3 p-3 bg-white dark:bg-zinc-900 rounded-2xl border transition-colors shadow-sm group touch-none relative ${isDragging ? "border-rose-500 shadow-xl scale-[1.02]" : "border-gray-100 dark:border-white/5"}`}>
            <div
                {...attributes}
                {...listeners}
                className="p-2 cursor-grab text-rose-400 hover:text-rose-600 active:scale-90 transition-transform">
                <GripVertical size={20} />
            </div>
            <div className="w-12 h-16 bg-gray-100 dark:bg-black rounded-lg overflow-hidden shrink-0 border border-gray-200 dark:border-zinc-800">
                <img
                    src={img.preview}
                    alt="P"
            ...
            <div className="flex-1 min-w-0">
                <p className="font-bold text-sm truncate dark:text-white">{img.file.name}</p>
            </div>
            <button
                onClick={() => onRemove(id)}
...
export default function ImageToPdfTool() {
    ...
    const handleFiles = (selectedFiles: FileList | File[]) => {
        ...
        if (newImages.length === 0) {
            toast.error("Select images (JPG, PNG, WebP)");
            return;
        }
    ...
    const ActionButton = () => (
        <button
            ...
            {isProcessing ? (
                ...
                <>
                    Generate PDF <ArrowRight size={18} />
                </>
            )}
        </button>
    );
    ...
    return (
        <NativeToolLayout
            title="Image to PDF"
            description="Convert photos and images into a professional PDF."
            actions={images.length > 0 && !downloadUrl && <ActionButton />}>
            <input
                type="file"
                multiple
                accept="image/*"
            ...
            {images.length === 0 ? (
                ...
                <div className="space-y-6">
                    <div className="flex justify-between items-center px-1">
                        <p className="text-[10px] font-black uppercase text-gray-400">{images.length} Images</p>
                        <button
                            onClick={() => setImages([])}
                            className="text-[10px] font-black uppercase text-rose-500/60">
                            Clear
                        </button>
                    </div>
                    <DndContext
                    ...
                    <button
                        onClick={() => fileInputRef.current?.click()}
                        className="w-full py-3 border-2 border-dashed border-gray-200 dark:border-zinc-800 rounded-2xl text-gray-400 font-bold text-sm flex items-center justify-center gap-2 hover:border-rose-500 hover:text-rose-500 transition-all">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className="w-full bg-gray-50 dark:bg-black rounded-xl px-4 py-3 border border-transparent focus:border-rose-500 outline-none font-bold text-sm"
                        />
                    </div>
