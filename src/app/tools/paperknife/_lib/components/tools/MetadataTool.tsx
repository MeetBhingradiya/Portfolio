...
import { Info, Lock, Edit3, Loader2, Sparkles, X } from "@/app/tools/paperknife/_lib/muiLucide";
...
export default function MetadataTool() {
    ...
    const ActionButtons = () => (
        <div className="flex flex-col gap-2">
            <button
                ...
                className={`w-full bg-rose-500 text-white font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 py-4 rounded-2xl text-sm md:p-6 md:rounded-3xl md:text-xl flex items-center justify-center gap-3 shadow-lg shadow-rose-500/20`}>
                {isProcessing && !isDeepCleaning ? <Loader2 className="animate-spin" /> : <Edit3 size={18} />} Update Metadata
            </button>
            <button
                onClick={() => saveMetadata(true)}
                disabled={isProcessing}
                className={`w-full bg-emerald-500 text-white font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 py-4 rounded-2xl text-sm md:p-6 md:rounded-3xl md:text-xl flex items-center justify-center gap-3 shadow-lg shadow-emerald-500/20`}>
    ...
    return (
        <NativeToolLayout
            ...
            description="Edit or wipe document properties for better privacy."
            actions={pdfData && !pdfData.isLocked && !downloadUrl && <ActionButtons />}>
            <input
                type="file"
                accept=".pdf"
                className="hidden"
