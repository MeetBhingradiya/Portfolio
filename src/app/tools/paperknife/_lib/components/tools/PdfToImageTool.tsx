...
import { Image as ImageIcon, Lock, Loader2, ArrowRight, X } from "@/app/tools/paperknife/_lib/muiLucide";
    ...
    const convertToImages = async () => {
        ...
        try {
            ...
            for (let i = 1; i <= pdfData.pageCount; i++) {
                ...
                zip.file(`${customFileName}-${padNum}.${format}`, base64Data, {
                    base64: true
                });
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white dark:bg-zinc-900 p-8 rounded-[2rem] border border-gray-100 dark:border-white/5 space-y-8 shadow-sm">
                        {!downloadUrl ? (
                            <>
                                <div>
                                    <label className="block text-[10px] font-black uppercase text-gray-400 mb-4 tracking-widest px-1">
                                        Image Format
                                    </label>
                                    <div className="grid grid-cols-2 gap-3">
                                        {(["jpg", "png"] as const).map((fmt) => (
                                            <button
                                                ...
                                                className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center ${format === fmt ? "border-rose-500 bg-rose-50/50 dark:bg-rose-900/10" : "border-gray-100 dark:border-white/5"}`}>
                                                <span
                                                    className={`font-black uppercase text-[10px] ${format === fmt ? "text-rose-500" : "text-gray-400"}`}>
                                                    {fmt}
                                                </span>
                                            </button>
                                        ))}
