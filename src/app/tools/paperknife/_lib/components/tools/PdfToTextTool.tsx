...
import { Loader2, Copy, FileText, Lock, Check, Download, Zap, ScanSearch, ArrowRight, X } from "@/app/tools/paperknife/_lib/muiLucide";
...
export default function PdfToTextTool() {
    ...
    // Optional OCR kill-switch for constrained deployments
    const isOcrDisabled = process.env.NEXT_PUBLIC_DISABLE_OCR === "true";
    ...
    return (
        <NativeToolLayout
            title="PDF to Text"
            description="Extract text using fast scan or deep local OCR."
            actions={pdfData && !pdfData.isLocked && !extractedText && <ActionButton />}>
            <input
                type="file"
                accept=".pdf"
                className="hidden"
                            ...
                            <>
                                <div className="grid grid-cols-2 gap-3">
                                    <button
                                        ...
                                        className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center ${extractionMode === "text" ? "border-rose-500 bg-rose-50/50 dark:bg-rose-900/10" : "border-gray-100 dark:border-white/5"}`}>
                                        <Zap
                                            size={20}
                                            className={extractionMode === "text" ? "text-rose-500" : "text-gray-400"}
                                        />
                                    ...
                                    <button
                                        ...
                                        <ScanSearch
                                            size={20}
                                            className={extractionMode === "ocr" ? "text-rose-500" : "text-gray-400"}
                                        />
                                        <span className="font-black uppercase text-[10px] mt-1">
                                            {isOcrDisabled ? "No OCR" : "Deep OCR"}
                                        </span>
                                    </button>
                                </div>
                                {isProcessing && (
                                        ...
                                        {isOcrDisabled && (
                                            <div className="p-4 bg-gray-50 dark:bg-zinc-900/50 rounded-xl border border-gray-100 dark:border-white/5">
                                                <p className="text-[9px] text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider text-center">
                                                    Deep OCR is disabled in this F-Droid build to comply with non-binary policies. Use 'Fast
                                                    Scan' or get the full version from GitHub.
                                                </p>
                                            </div>
                                        )}
                                        {extractionMode === "ocr" && !isOcrDisabled && (
                                            <div className="p-4 bg-amber-50 dark:bg-amber-900/10 rounded-xl border border-amber-100 dark:border-amber-900/20">
                                                <p className="text-[10px] text-amber-700 dark:text-amber-400 font-bold uppercase tracking-widest text-center">
                                                    Note: Deep OCR is CPU-intensive and may take a few minutes depending on your device
                                                    performance.
                                                </p>
                                            </div>
                                        )}
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <button
                                        ...
                                        <Download size={18} /> {isNative ? "Save .txt" : "Download"}
                                    </button>
                                </div>
                                <button
                                    onClick={() => {
