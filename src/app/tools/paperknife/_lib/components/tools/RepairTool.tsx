...
import { Loader2, ShieldAlert, Upload, X, FileCheck } from "@/app/tools/paperknife/_lib/muiLucide";
...
export default function RepairTool() {
    ...
    return (
        <NativeToolLayout
            title="Repair PDF"
            description="Fix corrupted or unreadable PDF files by rebuilding structure."
            actions={originalFile && !downloadUrl && <ActionButton />}>
            <input
                type="file"
                accept=".pdf"
                className="hidden"
            ...
            <div className="mt-8 p-6 bg-amber-50 dark:bg-amber-900/10 rounded-2xl border border-amber-100 dark:border-white/5 flex items-start gap-4">
                ...
                <div className="text-xs text-amber-700 dark:text-amber-400 leading-relaxed text-left">
                    ...
                    PaperKnife rebuilds the internal cross-reference table and regenerates the file structure from scratch. This can restore
                    access to many files that "cannot be opened."
                </div>
            </div>
            <PrivacyBadge />
        </NativeToolLayout>
