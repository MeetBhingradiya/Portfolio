...
import { Loader2, Scissors, Check, Plus, Lock, ArrowRight, X, Zap } from "@/app/tools/paperknife/_lib/muiLucide";
...
const LazyThumbnail = ({ pdfDoc, pageNum }: { pdfDoc: any; pageNum: number }) => {
    ...
    useEffect(() => {
        ...
        const observer = new IntersectionObserver(
            ...
            { rootMargin: "400px" }
        ); // Load ahead for smoother scrolling
    ...
    return (
        <div
            ref={imgRef}
            className="w-full h-full bg-gray-50 dark:bg-zinc-900 flex items-center justify-center text-xs font-bold text-gray-400">
                ...
                <button
                    ...
                    {isLoadingMeta ? (
                        <div className="flex flex-col items-center">
                            <Loader2
                                size={48}
                                className="text-rose-500 animate-spin mb-4"
                            />
                        ...
                        <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/20 p-4 rounded-2xl flex items-center gap-3 text-amber-600 dark:text-amber-400 animate-in fade-in duration-500">
                            ...
                            <div className="min-w-0">
                                ...
                                <p className="text-[11px] font-medium leading-tight">
                                    Visual selection for large files may be slower. Use range selection for speed.
                                </p>
                            </div>
                        </div>
                    )}
                                ...
                                <div className="flex justify-between items-center mb-6">
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            onClick={() => {
                                                const all = new Set<number>();
                                                for (let i = 1; i <= pdfData.pageCount; i++) all.add(i);
                                                setSelectedPages(all);
                                            }}
                                            className="text-[10px] font-black uppercase text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 px-2 py-1 rounded-lg transition-colors">
                                            Select All
                                        </button>
                                        <button
                                            onClick={() => setSelectedPages(new Set())}
                                            className="text-[10px] font-black uppercase text-gray-400 px-2 py-1 hover:bg-gray-50 dark:hover:bg-white/5 rounded-lg transition-colors">
                                            Clear
                                        </button>
                                    </div>
                                </div>
                                    ...
                                    {Array.from({
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`relative group cursor-pointer aspect-[3/4] rounded-xl overflow-hidden border-2 transition-all ${isSelected ? "border-rose-500 shadow-md scale-[1.02]" : "border-transparent hover:border-gray-200 dark:hover:border-zinc-800"}`}>
                                                <LazyThumbnail
                                                    pdfDoc={pdfData.pdfDoc}
                                                    pageNum={pageNum}
                                                />
                                                <div
                                                    className={`absolute inset-0 flex items-center justify-center transition-opacity ${isSelected ? "bg-rose-500/10 opacity-100" : "bg-black/20 opacity-0 group-hover:opacity-100"}`}>
                                                    <div
                                                        className={`w-8 h-8 rounded-full flex items-center justify-center shadow-lg transition-transform ${isSelected ? "bg-rose-500 text-white scale-100" : "bg-white text-gray-400 scale-75"}`}>
                                                        {isSelected ? (
                                                            <Check
                                                                size={20}
                                                                strokeWidth={3}
                            ...
                            <div className="bg-white dark:bg-zinc-900 p-8 rounded-[2rem] border border-gray-100 dark:border-white/5 shadow-sm sticky top-24">
                                <div className="space-y-6">
                                        ...
                                        <div>
                                            ...
                                            <div className="grid grid-cols-2 gap-2 bg-gray-50 dark:bg-black p-1 rounded-2xl">
                                                <button
                                                    ...
                                                    className={`py-2 px-3 rounded-xl text-[9px] font-black uppercase transition-all ${splitMode === "single" ? "bg-white dark:bg-zinc-800 text-rose-500 shadow-sm" : "text-gray-400"}`}>
                                                    One Document
                                                </button>
                                                <button
                                                    onClick={() => {
                                                    ...
                                                    className={`py-2 px-3 rounded-xl text-[9px] font-black uppercase transition-all ${splitMode === "individual" ? "bg-white dark:bg-zinc-800 text-rose-500 shadow-sm" : "text-gray-400"}`}>
                                                    Separate Files
                                                </button>
                                            </div>
                                        ...
                                        <div>
                                            ...
                                            <div className="flex gap-2">
                                                <input
                                                    ...
                                                    className="flex-1 bg-gray-50 dark:bg-black rounded-xl px-4 py-3 border border-transparent focus:border-rose-500 outline-none font-bold text-sm dark:text-white"
                                                />
                                                <button
                                                    onClick={() => parseRange(rangeInput)}
                                                    className="px-4 bg-rose-500 text-white rounded-xl font-black text-[10px] uppercase active:scale-95 transition-transform">
                                                    Apply
                                                </button>
                                            </div>
                                    ...
                                    <button
                                        ...
                                        className="w-full py-2 text-[10px] font-black uppercase text-gray-300 hover:text-rose-500 transition-colors">
                                        Close File
                                    </button>
                                </div>
                            </div>
