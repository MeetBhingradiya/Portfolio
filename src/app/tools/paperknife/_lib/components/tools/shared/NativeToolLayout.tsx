...
export const NativeToolLayout = ({ title, description, children, actions, onBack }: NativeToolLayoutProps) => {
        ...
        <div
            ...
            {showNativeHeader && (
                <header
                    ...
                    style={{
                        background: isApple ? (isDark ? "rgba(22,22,24,0.82)" : "rgba(255,255,255,0.78)") : palette.surface,
            ...
            {actions && (
                <div
                    ...
                    style={{
                        background: isApple ? (isDark ? "rgba(22,22,24,0.86)" : "rgba(255,255,255,0.84)") : palette.surface,
                        backdropFilter: isApple ? "blur(24px) saturate(165%)" : "none",
                        borderTop: `1px solid ${palette.border}`,
                        boxShadow: isApple ? "0 -8px 32px rgba(0,0,0,0.12)" : "0 -10px 40px rgba(0,0,0,0.05)"
                    }}>
                    <div className="p-4 max-w-md mx-auto">{actions}</div>
                </div>
            )}
        </div>
    );
