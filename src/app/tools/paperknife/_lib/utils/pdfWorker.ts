    ...
    try {
        if (type === "MERGE_PDFS") {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                self.postMessage({
                    type: "PROGRESS",
                    payload: Math.round(((i + 1) / files.length) * 100)
                });
            }
            ...
            if (mode === "single") {
                ...
                const sortedIndices = Array.from(selectedPages as number[])
                    .sort((a, b) => a - b)
                    .map((p) => p - 1);
                ...
                for (let i = 0; i < sortedPages.length; i++) {
                    ...
                    self.postMessage({
                        type: "PROGRESS",
                        payload: Math.round(((i + 1) / sortedPages.length) * 100)
                    });
                }
            ...
            for (let i = 0; i < pages.length; i++) {
                ...
                self.postMessage({
                    type: "PROGRESS",
                    payload: Math.round(((i + 1) / pages.length) * 100)
                });
            }
