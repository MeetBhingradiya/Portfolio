    ...
    const grouped = useMemo(() => {
        return PAPERKNIFE_CATEGORIES.map((category) => ({
            ...
            tools: PAPERKNIFE_TOOLS.filter((tool) => tool.category === category)
                .filter((tool) => isVisible(`paperknife-${tool.slug}`)),
        }));
    }, [visibility]);

    return (
        <main className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
            <div className="mb-8 rounded-3xl border p-6" style={{ borderColor: isDark ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.1)", background: isDark ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.02)" }}>
                <h1 className="text-3xl font-black tracking-tight" style={{ color: palette.textPrimary }}>PaperKnife Tools</h1>
            ...
            {!rootVisible && (
                <div className="mb-6 rounded-2xl border px-4 py-3 text-sm font-semibold" style={{ borderColor: isDark ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.1)", color: palette.textSecondary }}>
                    PaperKnife is currently disabled by admin settings.
                </div>
            )}

            {rootVisible && <div className="space-y-8">
                {grouped.map((section) => (
                    <section key={section.category}>
                        <h2 className="mb-3 text-lg font-black" style={{ color: palette.textPrimary }}>{section.category}</h2>
                        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                            {section.tools.map((tool) => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className="mb-2 inline-flex h-10 w-10 items-center justify-center rounded-xl" style={{ background: palette.accentSubtle, color: palette.accent }}>
                                            <Icon size={18} />
                                        </div>
