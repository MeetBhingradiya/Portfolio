...
import {
    Dashboard, ChecklistRtl, Loop, TrackChanges, Notifications,
    ArrowBack, Menu, Close, Rocket, GetApp,
} from "@mui/icons-material";

const ACCENT = "#AF52DE";

const NAV_ITEMS = [
    { label: "Dashboard",  href: "/tools/productivity",            icon: <Dashboard  fontSize="small" /> },
    { label: "Tasks",      href: "/tools/productivity/tasks",      icon: <ChecklistRtl fontSize="small" /> },
    { label: "Habits",     href: "/tools/productivity/habits",     icon: <Loop       fontSize="small" /> },
    { label: "Goals",      href: "/tools/productivity/goals",      icon: <TrackChanges fontSize="small" /> },
    { label: "Reminders",  href: "/tools/productivity/reminders",  icon: <Notifications fontSize="small" /> },
];
...
export default function ProductivityLayout({ children }: { children: React.ReactNode }) {
    ...
    typeof window !== "undefined" && !installPrompt && window.addEventListener("beforeinstallprompt", (e: any) => {
        e.preventDefault();
        setInstallPrompt(e);
    }, { once: true });

    const surfaceBg   = isApple
        ? isDark ? "rgba(28, 28, 30, 0.95)" : "rgba(255, 255, 255, 0.92)"
        : palette.surface;
        ...
        <div className="flex flex-col h-full p-4 gap-1">
            ...
            <div className="flex items-center gap-3 px-3 py-4 mb-2">
                ...
                <div className="min-w-0">
                    <p className="font-bold text-sm leading-tight truncate" style={{ color: palette.textPrimary }}>
                        Productivity Hub
                    </p>
                    <p className="text-xs truncate" style={{ color: palette.textSecondary }}>Gamified · Personal</p>
                </div>
            </div>
                    ...
                    return (
                        <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)}>
                            <motion.div
                                ...
                                whileTap={{ scale: 0.98 }}
                            >
                                <span style={{ color: active ? ACCENT : palette.textTertiary }}>
                                    {item.icon}
                                </span>
            ...
            <Link href="/settings">
                <motion.div
                    ...
                    whileHover={{ color: palette.textSecondary, background: isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.04)" }}
                    whileTap={{ scale: 0.98 }}
                >
    ...
    return (
        <>
            ...
            <div className="flex min-h-screen min-h-[100dvh]" style={{ background: palette.background }}>
                {/* Desktop sidebar */}
                <aside
                    className="hidden lg:flex flex-col w-60 shrink-0 sticky top-0 h-screen"
                    style={{ background: surfaceBg, borderRight: `1px solid ${borderColor}`, backdropFilter: isApple ? "blur(24px)" : "none" }}
                >
                    <SidebarContent />
                </aside>
