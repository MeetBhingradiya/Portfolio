...
function fmtDateTime(d: string) {
    ...
    const time = dt.toLocaleTimeString("en", {
        hour: "2-digit",
        minute: "2-digit"
    });
...
function ReminderItem({
    ...
    return (
        <motion.div
            ...
            animate={{ opacity: dismissed ? 0.55 : 1, y: 0 }}
            exit={{ opacity: 0, height: 0 }}>
            {/* Priority stripe */}
            <div
                className="absolute left-0 top-3 bottom-3 w-0.5 rounded-full"
                style={{ background: pColor, borderRadius: 4 }}
            />
            ...
            <div className="flex items-start gap-3 pl-2">
                ...
                <div className="shrink-0 mt-0.5">
                    {dismissed ? (
                        ...
                        <Snooze style={{ color: "#f59e0b", fontSize: 20 }} />
                    ) : overdue ? (
                        <ErrorOutline style={{ color: "#ef4444", fontSize: 20 }} />
                    ) : today ? (
                        <Alarm style={{ color: "#3b82f6", fontSize: 20 }} />
                    ) : (
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {reminder.Description && (
                        <p
                            className="text-xs line-clamp-1 mt-0.5 leading-relaxed"
                    ...
                    <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                        <span
                            ...
                            style={{
                                color: overdue ? "#ef4444" : today ? "#3b82f6" : palette.textTertiary
                            }}>
                        ...
                        {reminder.Repeat !== "NONE" && (
                            <span
                                ...
                                <Loop style={{ fontSize: 10 }} />
                                {REPEAT_LABELS[reminder.Repeat as keyof typeof REPEAT_LABELS] ?? reminder.Repeat}
                            </span>
                        )}
                        <span
                        ...
                        {snoozed && reminder.SnoozedUntil && (
                            <span
                                className="text-xs"
                                style={{ color: "#f59e0b" }}>
                                until {fmtDateTime(reminder.SnoozedUntil)}
                            </span>
                        )}
                    </div>
                </div>
                ...
                <div className="flex items-center gap-1 shrink-0">
                        ...
                        <>
                            ...
                            <div className="relative">
                                ...
                                <AnimatePresence>
                                    {snoozeOpen && (
                                        <motion.div
                                            className="absolute right-0 top-full mt-1 rounded-xl shadow-xl z-50 overflow-hidden py-1 w-28"
                                            style={{
                                            ...
                                            {SNOOZE_OPTIONS.map((m) => (
                                                <button
                                                    key={m}
                                                    onClick={() => {
                                                        onSnooze(reminder.ReminderID, m);
                                                        setSnoozeOpen(false);
                                                    }}
                    ...
                    <div
                        ...
                        <AnimatePresence>
                            {menuOpen && (
                                <motion.div
                                    className="absolute right-0 top-full mt-1 w-36 rounded-xl shadow-xl z-50 overflow-hidden py-1"
                                    style={{
                                        background: isDark ? "rgba(28,28,30,0.98)" : "rgba(255,255,255,0.98)",
                                        border: `1px solid ${border}`,
                                        backdropFilter: "blur(16px)"
                                    ...
                                    <button
                                        ...
                                        className="flex items-center gap-2 w-full px-3 py-2 text-sm hover:bg-red-500/10"
                                        style={{ color: "#ef4444" }}>
            ...
            <motion.div
                ...
                <div className="flex items-center justify-between mb-5">
                    <h2
                        ...
                        style={{ color: palette.textPrimary }}>
                        {initial?.ReminderID ? "Edit Reminder" : "New Reminder"}
                    </h2>
                ...
                {aiMode && (
                    <div className="mb-4 flex gap-2">
                        <input
                            ...
                            placeholder="e.g. remind me to exercise tomorrow at 7am"
                        ...
                        <div className="flex gap-2 flex-wrap">
                            {["NONE", "DAILY", "WEEKLY", "MONTHLY", "YEARLY"].map((r) => (
                                <button
                                    ...
                                    style={{
                                        background: repeat === r ? `${ACCENT}20` : isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.04)",
                                        color: repeat === r ? ACCENT : palette.textTertiary,
                                        border: `1px solid ${repeat === r ? ACCENT : border}`
                                    }}>
                                        ...
                                        style={{
                                            background: days.includes(i)
                                                ...
                                                : isDark
                                                  ? "rgba(255,255,255,0.05)"
                                                  : "rgba(0,0,0,0.04)",
                                            color: days.includes(i) ? ACCENT : palette.textTertiary,
                                            border: `1px solid ${days.includes(i) ? ACCENT : border}`
                                        }}>
                                        {n[0]}
                                    </button>
                                ))}
                    ...
                    <button
                        onClick={() => {
                            ...
                            onSave({
                                ...
                                ScheduledAt: new Date(dtLocal).toISOString(),
                                Repeat: repeat,
...
export default function RemindersPage() {
        ...
        try {
            ...
            if (j.success) {
                const sorted = [...(j.data ?? [])].sort(
                    (a: Reminder, b: Reminder) => new Date(a.ScheduledAt).getTime() - new Date(b.ScheduledAt).getTime()
                );
                setReminders(sorted);
            }
        } finally {
            ...
            for (const rem of (j.data ?? []) as Reminder[]) {
                ...
                if (at <= now && at > new Date(now.getTime() - 60_000)) {
                    new Notification(rem.Title, {
                        body: rem.Description ?? fmtDateTime(rem.ScheduledAt),
                        icon: "/favicon.ico"
                    });
                }
            }
    ...
    async function saveReminder(data: Partial<Reminder>) {
        if (editReminder?.ReminderID) {
            await fetch(`/api/productivity/reminders/${editReminder.ReminderID}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(data)
            });
        } else {
            await fetch("/api/productivity/reminders", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(data)
            });
        ...
        <div className="p-4 md:p-6 max-w-3xl mx-auto pb-24 space-y-5">
            ...
            {notifPerm !== null && notifPerm !== "granted" && (
                <motion.div
                    ...
                    style={{
                        background: isDark ? "rgba(251,191,36,0.12)" : "rgba(251,191,36,0.10)",
                    ...
                    <div
                        ...
                        {notifPerm === "denied" ? (
                            <NotificationsOff style={{ color: "#f59e0b" }} />
                        ) : (
                            <Notifications style={{ color: "#f59e0b" }} />
                        )}
                    </div>
            ...
            {loading ? (
                ...
                <div className="text-center py-16 space-y-3">
                    ...
                    <p
                        className="text-sm"
                        style={{ color: palette.textSecondary }}>
                        Schedule reminders that notify you at the right time.
                    </p>
                </div>
            ) : (
                    ...
                    {tab === "ACTIVE" && today.filter((r) => !isOverdue(r.ScheduledAt, r.Status)).length > 0 && (
                        <div>
                            ...
                            <div className="space-y-2">
                                <AnimatePresence mode="popLayout">
                                    {today
                                        .filter((r) => !isOverdue(r.ScheduledAt, r.Status))
                                        .map((r) => (
                                            <ReminderItem
                                                key={r.ReminderID}
                                                reminder={r}
