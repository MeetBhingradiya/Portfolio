...
import { ArrowBack, AutoGraph, CalendarMonth, CheckCircle, Delete, EmojiEvents, Refresh, Warning } from "@mui/icons-material";
...
LicenseInfo.setLicenseKey(process.env.NEXT_PUBLIC_MUI_X_LICENSE_KEY?.trim() || fallbackLicense);
    ...
    return (
        <Card
            ...
            <CardContent>
                ...
                <Typography
                    variant="caption"
                    sx={{ color: text, display: "block", mt: 1 }}>
                    {sub}
                </Typography>
            </CardContent>
        </Card>
...
export default function AnalyticsPage() {
    ...
    const surfaceBg = isApple ? (isDark ? "rgba(30, 30, 34, 0.58)" : "rgba(255, 255, 255, 0.72)") : palette.surface;
    ...
    const chartPalette = useMemo(() => ["#34d399", "#fb7185", "#f59e0b", "#60a5fa", "#a78bfa", "#2dd4bf", "#f97316", "#14b8a6"], []);
    ...
    const fetchDailyCapital = useCallback(async () => {
        ...
        if (res.success) {
            setViewState((prev) => ({
                ...prev,
                dcEntries: res.data as DailyCapitalEntry[]
            }));
        }
    }, []);
    ...
    const fetchAnalytics = useCallback(async () => {
        ...
        try {
            ...
            if (json.success) {
                setViewState((prev) => ({
                    ...prev,
                    data: json.data as AnalyticsData
                }));
            }
        } catch (err) {
            console.error(err);
    ...
    const handleDeleteDc = async (date: string) => {
        ...
        await fetch(`/api/trade-journal/daily-capital?date=${date}`, {
            method: "DELETE"
        });
        ...
        ? [
              {
                  title: "Net P&L",
                  value: `₹${fmt(s.netPnL)}`,
                  sub: `Gross ₹${fmt(s.grossWin)} / Loss ₹${fmt(s.grossLoss)}`,
                  tone: pnlTone(s.netPnL)
              },
              {
                  title: "Daily Avg P&L",
                  value: `₹${fmt(s.dailyAvgPnL)}`,
                  sub: "Net per trading day",
                  tone: pnlTone(s.dailyAvgPnL)
              },
              {
                  title: "Win Rate",
                  value: `${fmt(s.winRate)}%`,
                  sub: `${s.wins}W ${s.losses}L ${s.breakeven}BE`,
                  tone: s.winRate >= 50 ? "#22c55e" : "#f59e0b"
              },
              {
                  title: "Profit Factor",
                  value: s.profitFactor ? s.profitFactor.toFixed(2) : "-",
                  sub: "Gross Win / Gross Loss",
                  tone: s.profitFactor >= 1.5 ? "#22c55e" : "#f59e0b"
              },
              {
                  title: "Highest P&L",
                  value: `₹${fmt(s.highestPnL, 0)}`,
                  sub: "Best trade",
                  tone: pnlTone(s.highestPnL)
              },
              {
                  title: "Lowest P&L",
                  value: `₹${fmt(s.lowestPnL, 0)}`,
                  sub: "Worst trade",
                  tone: pnlTone(s.lowestPnL)
              },
              {
                  title: "Average Charges",
                  value: `₹${fmt(s.avgCharges)}`,
                  sub: "Brokerage + taxes per trade",
                  tone: "#f59e0b"
              },
              {
                  title: "Today Charges",
                  value: `₹${fmt(s.todayCharges)}`,
                  sub: "Today brokerage + taxes",
                  tone: s.todayCharges > 0 ? "#ef4444" : "#9ca3af"
              }
          ]
        : [];
                        ...
                        <CardContent sx={{ p: cardPadding }}>
                            ...
                            {equitySeries.length ? (
                                <LineChart
                                    ...
                                    yAxis={[
                                        {
                                            valueFormatter: (v: number | null) => `₹${fmt((v ?? 0) as number, 0)}`
                                        }
                                    ]}
                                    series={[
                                        {
                                            data: equitySeries,
                                            area: true,
                                            color: pnlTone(equitySeries[equitySeries.length - 1] ?? 0),
                                            showMark: false
                                        }
                                    ]}
                                    ...
                                    series={[
                                        {
                                            ...
                                            color: isApple ? palette.accent : "#4f46e5"
                                        }
                                    ]}
                                    grid={{ horizontal: true }}
                                />
                            ...
                            {instrumentTop.length ? (
                                <BarChart
                                    ...
                                    yAxis={[
                                        {
                                            scaleType: "band",
                                            data: instrumentTop.map((i) => i.instrument)
                                        }
                                    ]}
                                    xAxis={[
                                        {
                                    ...
                                    series={[
                                        {
                                            data: instrumentTop.map((i) => i.pnl),
                                            color: isApple ? "#60a5fa" : "#0ea5e9"
                                        }
                                    ]}
                                    grid={{ vertical: true }}
                                />
                            ...
                            {setupTop.length ? (
                                <BarChart
                                    ...
                                    yAxis={[
                                        {
                                            scaleType: "band",
                                            data: setupTop.map((i) => i.setup.replace(/_/g, " "))
                                        }
                                    ]}
                                    xAxis={[
                                        {
                                    ...
                                    series={[
                                        {
                                            data: setupTop.map((i) => i.pnl),
                                            color: isApple ? "#14b8a6" : "#22c55e"
                                        }
                                    ]}
                                    grid={{ vertical: true }}
                                />
                            ...
                            {mistakeData.length ? (
                                <BarChart
                                    ...
                                    yAxis={[
                                        {
                                            scaleType: "band",
                                            data: mistakeData.map((i) => i.mistake.replace(/_/g, " "))
                                        }
                                    ]}
                                    xAxis={[{}]}
                                    series={[
                                        {
                                            data: mistakeData.map((i) => i.count),
                                            color: isApple ? "#fb7185" : "#ef4444"
                                        }
                                    ]}
                                    grid={{ vertical: true }}
                                />
                    ...
                    <Card sx={{ mt: 1.5, ...panelSx }}>
                        <CardContent sx={{ p: cardPadding }}>
                            <Stack
                                ...
                                <Chip
                                    color={s.edgeScore >= 6 ? "success" : s.edgeScore >= 4 ? "warning" : "error"}
                                    label={`${s.edgeScore}/${s.edgeScoreMax} Rules Passing`}
                                />
                            </Stack>
                            <Divider sx={{ my: 1.5 }} />
                            <Box
                                            ...
                                            return (
                                                <TableRow key={entry.Date}>
                                                    ...
                                                    <TableCell
                                                        ...
                                                        {fmt(entry.StartingCapital, 0)}
                                                    </TableCell>
                                                    <TableCell
                                                        sx={{
                                                            color: palette.textPrimary
                                                        }}>
                                                        {fmt(entry.EndingCapital, 0)}
                                                    </TableCell>
                                                    <TableCell
                                                        sx={{
                                                    ...
                                                    <TableCell align="right">
                                                        <IconButton
                                                            color="error"
                                                            size="small"
                                                            disabled={dcDeleting === entry.Date}
                                                            onClick={() => handleDeleteDc(entry.Date)}>
