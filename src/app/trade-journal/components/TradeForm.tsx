...
function estimateCharges(segment: string, entryPrice: number, exitPrice: number, qty: number, direction: string): ChargesDetail {
...
const MERIDIEM_SELECT_OPTS = [
    { value: "", label: "AM/PM" },
    { value: "AM", label: "AM" },
    { value: "PM", label: "PM" }
];

function TimePickerAmPm({ value, onChange }: { value?: string; onChange: (value: string) => void }) {
...
export default function TradeForm({ initialData, onSubmit, submitting, isEdit, enableBackendDraft = false }: Props) {
    ...
    const [directionTouched, setDirectionTouched] = useState(Boolean(initialData?.PositionDuration || initialData?.Direction));
    ...
    useEffect(() => {
        fetch("/api/trade-journal/instruments")
            .then((r) => r.json())
            .then((j) => {
                if (j.success) setInstrumentSuggestions(j.data);
            })
            .catch(() => {});
    }, []);
    ...
    const effectiveDirection = directionTouched ? (selectedDirection === "LONG" ? "LONG" : "SHORT") : inferredDirection;

    const instrumentPreview = useMemo(() => {
        const parts = [
            String(form.InstrumentName || "")
                .trim()
                .toUpperCase()
        ];
    ...
    const calculatedPnl = useMemo(() => {
        ...
        const effectiveQty = Number.isFinite(qty) && qty > 0 ? (Number.isFinite(lot) && lot > 1 ? Math.floor(qty / lot) * lot : qty) : 0;

        if (!Number.isFinite(entry) || !Number.isFinite(exit) || effectiveQty <= 0) {
            return { amount: "", sign: "PROFIT" };
        }

        const raw = effectiveDirection === "SHORT" ? (entry - exit) * effectiveQty : (exit - entry) * effectiveQty;

        return {
            amount: Math.abs(Number(raw.toFixed(2))),
            sign: raw >= 0 ? "PROFIT" : "LOSS"
        };
    ...
    const defaultExpirySuggestion = useMemo(() => getInstrumentDefaultExpiry(String(form.InstrumentName || "")), [form.InstrumentName]);
    ...
    const effectiveTaxes = useMemo(
        () =>
            r2(
                calculatedCharges.stt +
                    calculatedCharges.stampDuty +
                    calculatedCharges.exchangeTurnover +
                    calculatedCharges.sebiTurnover +
                    effectiveGst
            ),
        [
            calculatedCharges.exchangeTurnover,
            ...
            <form
                ...
                <motion.div
                    ...
                    style={{
                        ...
                        border: `1.5px dashed ${palette.accent}50`
                    }}
                    ...
                    <>
                        ...
                        <Field label="Entry Time *">
                            <TimePickerAmPm
                                ...
                                onChange={(v) => setField("EntryTime", v)}
                            />
                        ...
                        <Field label="Instrument Name *">
                            <input
                                ...
                                value={String(form.InstrumentName)}
                                onChange={(e) => setField("InstrumentName", e.target.value.toUpperCase())}
                                required
                            />
                            <datalist id="instrument-suggestions">
                                {instrumentSuggestions.map((v) => (
                                    <option
                                        key={v}
                                        value={v}
                                    />
                            ...
                            <details
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-3">
                                    <Field label="Exit Time">
                                        <TimePickerAmPm
                                            value={form.ExitTime}
                                            onChange={(v) => setField("ExitTime", v)}
                                        />
                                    ...
                                    <Field label="Option Type">
                                        <CustomSelect
                                            ...
                                            onChange={(v) => setField("OptionType", v)}
                            ...
                            <details
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-3">
                                    ...
                                    <Field label="P&L Sign Override">
                                        <CustomSelect
                                            value={overridePnlSign}
                                            onChange={(v) => setOverridePnlSign(v)}
                                            options={PNL_SIGN_OPTS}
                                        />
                                    </Field>
                        ...
                        <div className="sm:col-span-2 lg:col-span-3">
                            <p
                                ...
                                {effectiveTotalCharges > 0 && (
                                    <span
                                        className="ml-2 font-bold text-sm normal-case tracking-normal"
                                        style={{ color: "#ef4444" }}>
                                ...
                                return (
                                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
                                        {(
                                            [
                                                {
                                                    label: `Brokerage${overrideBrokerageValue != null ? " (override)" : ""}`,
                                                    hint: "₹20 buy + ₹20 sell",
                                                    value: effectiveBrokerage
                                                },
                                                {
                                                    label: "STT",
                                                    hint: "Securities Transaction Tax",
                                                    value: calculatedCharges.stt
                                                },
                                                {
                                                    label: "Stamp Duty",
                                                    hint: "On buy-side notional",
                                                    value: calculatedCharges.stampDuty
                                                },
                                                {
                                                    label: "Exchange Turnover",
                                                    hint: "NSE / MCX fees",
                                                    value: calculatedCharges.exchangeTurnover
                                                },
                                                {
                                                    label: "SEBI Turnover",
                                                    hint: "₹10 per crore",
                                                    value: calculatedCharges.sebiTurnover
                                                },
                                                {
                                                    label: `GST (${isHighGstSegment ? "30%" : "18%"})${overrideGstValue != null ? " (override)" : ""}`,
                                                    hint: "On brokerage + exchange + SEBI",
                                                    value: effectiveGst
                                                }
                                            ] as const
                                            ...
                                            <div
                                                ...
                                                <span
                                                    ...
                                                    style={{
                                                        color: value > 0 ? "#ef4444" : palette.textSecondary
                                                    }}>
                                                    ₹
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-3">
                                    <Field label="Brokerage Override">
                                        ...
                                        <p
                                            ...
                                            Auto: ₹
                                            {calculatedCharges.brokerage.toLocaleString("en-IN", {
                                                maximumFractionDigits: 2,
                                                minimumFractionDigits: 2
                                            })}
                                        </p>
                                    ...
                                    <Field label="GST Override">
                                        ...
                                        <p
                                            ...
                                            Auto: ₹
                                            {calculatedCharges.gst.toLocaleString("en-IN", {
                                                maximumFractionDigits: 2,
                                                minimumFractionDigits: 2
                                            })}
                                        </p>
                                    ...
                                    <Field label="Total Charges Override">
                                        ...
                                        <p
                                            ...
                                            Auto: ₹
                                            {autoTotalCharges.toLocaleString("en-IN", {
                                                maximumFractionDigits: 2,
                                                minimumFractionDigits: 2
                                            })}
                                        </p>
                                    ...
                                    <Field label="Margin Used Override">
                                        ...
                                        <p
                                            ...
                                            {marginUsed == null
                                                ? "-"
                                                : `₹${marginUsed.toLocaleString("en-IN", { maximumFractionDigits: 2, minimumFractionDigits: 2 })}`}
                                        </p>
                                    </Field>
                                    <Field label="Net P&L Override">
                                        ...
                                        <p
                                            ...
                                            Auto:{" "}
                                            {autoNetPnlAfterCharges == null
                                                ? "-"
                                                : `${autoNetPnlAfterCharges >= 0 ? "+" : ""}₹${Math.abs(autoNetPnlAfterCharges).toLocaleString("en-IN", { maximumFractionDigits: 2, minimumFractionDigits: 2 })}`}
                                        </p>
                                    </Field>
                                </div>
                            </details>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-3">
                                    <Field label="Setup Type">
                                        <input
                                            ...
                                            onChange={(e) => setField("SetupType", e.target.value.toUpperCase())}
                                        />
                                        <datalist id="setup-suggestions">
                                            {SETUP_SUGGESTIONS.map((v) => (
                                                <option
                                                    key={v}
                                                    value={v}
                                                />
                                    ...
                                    <Field label="Market Condition">
                                        <input
                                            ...
                                            onChange={(e) => setField("MarketCondition", e.target.value.toUpperCase())}
                                        />
                                        <datalist id="market-suggestions">
                                            {MARKET_SUGGESTIONS.map((v) => (
                                                <option
                                                    key={v}
                                                    value={v}
                                                />
                                    ...
                                    <Field label="Emotional State">
                                        <input
                                            ...
                                            onChange={(e) => setField("EmotionalState", e.target.value.toUpperCase())}
                                        />
                                        <datalist id="emotion-suggestions">
                                            {EMOTION_SUGGESTIONS.map((v) => (
                                                <option
                                                    key={v}
                                                    value={v}
                                                />
                                    ...
                                    <Field label="Mistake Type">
                                        <input
                                            list="mistake-suggestions"
                                            type="text"
                                            style={inputStyle}
                                            value={form.MistakeType ?? ""}
                                            onChange={(e) => setField("MistakeType", e.target.value.toUpperCase())}
                                        />
                                        <datalist id="mistake-suggestions">
                                            {MISTAKE_SUGGESTIONS.map((v) => (
                                                <option
                                                    key={v}
                                                    value={v}
                                                />
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-3">
                                    <Field label="Attachment Links (comma-separated)">
                                        <input
                                            ...
                                            onChange={(e) => setField("AttachmentLinks", e.target.value)}
                                        />
                                    </Field>
                                    <div>
