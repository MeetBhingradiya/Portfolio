...
type AccountAction =
    ...
    | {
          type: "UPDATE_ACCOUNT";
          payload: { userID: string; updates: Partial<Account> };
      }
    ...
    switch (action.type) {
        ...
        case "ADD_ACCOUNT":
            ...
            return {
                ...
                accounts: [
                    ...state.accounts,
                    { ...action.payload, LastUsed: new Date() }
                ]
            };
...
const StorageUtils = {
    ...
    getActiveAccountID(): string {
        const activeUserID = localStorage.getItem(
            StorageKeys.ActiveSessionUserID
        );
        return activeUserID || "";
    },
...
export function AccountProvider({ children }: AccountProviderProps) {
    ...
    const validateAccount = useCallback(
        async (account: Account): Promise<boolean> => {
            try {
                ...
                log(
                    `ERROR : Failed to validate account ${account.UserID}:`,
                    error
                );
                return false;
            }
    ...
    const validateAllAccounts = useCallback(async (): Promise<void> => {
        ...
        if (validAccounts.length > 0) {
            if (
                activeAccountID &&
                validAccounts.some((acc) => acc.UserID === activeAccountID)
            ) {
            ...
            const activeAccount = validAccounts.find(
                (acc) => acc.UserID === newActiveAccountID
            );
            if (activeAccount) {
                StorageUtils.setActiveAccountID(newActiveAccountID);
                StorageUtils.setAuthenticationCookie(
                    activeAccount.Session.Token
                );
            }
        } else {
        ...
        log(
            `INFO : Account validation completed. ${validAccounts.length} valid accounts found.`
        );
    }, [validateAccount]);
        ...
        if (state.isInitialized) {
            ...
            if (state.activeAccountID) {
                const activeAccount = state.accounts.find(
                    (acc) => acc.UserID === state.activeAccountID
                );
                if (activeAccount) {
                    StorageUtils.setAuthenticationCookie(
                        activeAccount.Session.Token
